...
/**
 * This class supports writing of images as frames into the video track of a
 * QuickTime movie file.
 * <p>
 * All frames are encoded either using the JPG or the PNG video format. Each
 * frame can have an individual encoding quality and duration.
 * <p>
 * For detailed information about the QuickTime file format see:
 * http://developer.apple.com/documentation/QuickTime/QTFF/
 * 
 * @author Werner Randelshofer modified by David Bahr
 * @version 1.1 2008-08-11 Streamlined source code and API with AVIOutputStream.
 *          <br>
 *          1.0.1 2008-06-18 WideDataAtom wrote incorrect header for atoms
 *          larger than 4 GB. The default value of time scale is now 600.
 *          Renamed method writeFrame to writeFrame. Added writeFrame methods
 *          which take a file or an input stream as argument. <br>
...
public class QuickTimeOutputStream
{
	...
	/**
	 * Sets the frames per second or time scale for this media, that is, the
	...
	public void setFrameRate(int frameRate)
	{
		...
		this.timeScale = frameRate;
	}

	/**
	 * Returns the frames per second or time scale of this media.
	 * 
	 * @return time scale
	 */
	...
	/**
	 * Sets the state of the QuickTimeOutpuStream to States.STARTED.
	 * <p>
			...
			throws IOException
	{
		...
		{
		case JPG:
			iw = (ImageWriter) ImageIO.getImageWritersByMIMEType("image/jpeg")
					.next();
		...
		case PNG:
		default:
			iw = (ImageWriter) ImageIO.getImageWritersByMIMEType("image/png")
					.next();
			iwParam = iw.getDefaultWriteParam();
			break;
		}
	 ...
	 * <p>
	 * This method does not inspect the contents of the file. Its your
	 * responsibility to only add JPG files if you have chosen the JPEG video
	 * format, and only PNG files if you have chosen the PNG video format.
	 * <p>
	...
	{
		/*
		 * File type atom typedef struct { magic brand; bcd4 versionYear; bcd2
		 * versionMonth; bcd2 versionMinor; magic[4] compatibleBrands; }
		 * ftypAtom;
		 */
	...
	private void writeEpilog() throws IOException
	{
		 ...
		 * scale and duration. It has an atom type value of 'mvhd'. typedef
		 * struct { byte version; byte[3] flags; mactimestamp creationTime;
		 * mactimestamp modificationTime; int timeScale; int duration; int
		 * preferredRate; short preferredVolume; byte[10] reserved; int[9]
		 * matrix; int previewTime; int previewDuration; int posterTime; int
		 * selectionTime; int selectionDuration; int currentTime; int
		 * nextTrackId; } movieHeaderAtom;
		 */
		...
		// units. Note that this property is derived from the movies tracks.
		...
		// movies sound. A value of 1.0 indicates full volume.
		 ...
		 * 'tkhd'). typedef struct { byte version; byte flag0; byte flag1; byte
		 * set TrackHeaderFlags flag2; mactimestamp creationTime; mactimestamp
		...
		// 0x0002.
		// Track in preview
		// Indicates that the track is used in the movies preview. Flag
		// value is 0x0004.
		// Track in poster
		// Indicates that the track is used in the movies poster. Flag
		// value is 0x0008.
		...
		// movies time coordinate system). Note that this property is derived
		// from the tracks edits. The value of this field is equal to the sum
		// of the durations of all of the tracks edits. If there is no edit
		...
		// A 16-bit integer that indicates this tracks spatial priority in its
		...
		// A 16-bit fixed-point value that indicates how loudly this tracks
		...
		d.writeInt(timeScale); // timeScale
		// A time value that indicates the time scale for this mediathat is,
		// the number of time units that pass per second in its time coordinate
		// system.
		...
		// A 16-bit integer that specifies the medias playback qualitythat is,
		...
		// datafor example, 'vide' for video data or 'soun' for sound data.
		//
		// For data handlers, this field defines the data reference typefor
		...
		// A (counted) string that specifies the name of the componentthat is,
		...
		// intended for playback using version 1.0 of QuickTime. This flags
		// value is 0x0001.
		...
		// is to be used to interpret the medias data. The handler reference
		...
		// handler component how to access the medias data.
		 ...
		 * dataReferenceAtom; set { dataRefSelfReference=1 // I am not shure if
		 * this is the correct value for this flag } drefEntryFlags; typedef
		 * struct { int size; magic type; byte version; ubyte flag1; ubyte
		...
		//
		// Self reference
		// This flag indicates that the medias data is in the same file as
		...
		// a different fork from the movie atom. This flags value is
		// 0x0001.
		 ...
		 * sampleDescriptionAtom; typedef struct { int size; magic type; byte[6]
		 * reserved; // six bytes that must be zero short dataReferenceIndex; //
		 * A 16-bit integer that contains the index of the data reference to use
		 * to retrieve data associated with samples that use this sample
		 * description. Data references are stored in data reference atoms.
		 * byte[size - 16] data; } sampleDescriptionEntry;
		 */
		d = leaf.getOutputStream();
		d.write(0); // version
		...
		/* Time to Sample atom ---- */
		// Time-to-sample atoms store duration information for a medias
		 ...
		 * timeToSampleAtom; typedef struct { int sampleCount; int
		 * sampleDuration; } timeToSampleTable;
		 */
		d = leaf.getOutputStream();
		d.write(0); // version
		 ...
		 * sampleToChunkAtom; typedef struct { int firstChunk; int
		 * samplesPerChunk; int sampleDescription; } sampleToChunkTable;
		 */
		d = leaf.getOutputStream();
		d.write(0); // version
		// A 1-byte specification of the version of this time-to-sample atom.
		 ...
		 * sampleSizeAtom; typedef struct { int size; } sampleSizeTable;
		 */
		d = leaf.getOutputStream();
		d.write(0); // version
		// A 1-byte specification of the version of this time-to-sample atom.
		...
		if(videoFrames.getLast().offset <= 0xffffffffL)
		{
			 ...
			 * chunkOffsetTable[numberOfEntries]; } chunkOffsetAtom; typedef
			 * struct { int offset; } chunkOffsetTable;
			 */
			d = leaf.getOutputStream();
			d.write(0); // version
			...
			for(Sample s : videoFrames)
			{
				...
				// numberthe first table entry corresponds to the first chunk,
		...
		else
		{
			 ...
			 * typedef struct { byte version; byte[3] flags; int
			 * numberOfEntries; chunkOffsetTable
			 * chunkOffset64Table[numberOfEntries]; } chunkOffset64Atom; typedef
			 * struct { long offset; } chunkOffset64Table;
			 */
			d = leaf.getOutputStream();
			d.write(0); // version
			// A 1-byte specification of the version of this time-to-sample
