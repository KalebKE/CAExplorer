/*
 TunnellingSpaceships -- a class within the Cellular Automaton Explorer. 
...
/**
 * A rule which produces spaceship-like lines, and a wide variety of other
 * behaviors depending on the number of states and the lattice. For some of the
 * best spaceships choose a square (Margolus) 100 by 100 lattice with 31 states
 * and a 50% random initial state. Some states tunnel through others with no
 * effect, while others tunnel and eat their surroundings as they go. Other
 * choices for the number of states look like Majority Probably Wins, simple
 * translations, or premature death. In one-dimension, the rule produces complex
 * streams of colliding data that usually end in Class II cycles.
 * <p>
 * How's the rule work? The neighbors (not including the cell) are concatenated
 * into a single value. This value is converted to a base 10 integer. The
 * neighbor that corresponds to that integer (modulo the number of neighbors) is
 * selected as the cell's new value.
 * <p>
 * For example, in one dimension with nearest neighbors and two states (0 and
 * 1), the left and right neighbors may each have values 1. These concatenate to
 * 11 in base 2, which is the integer 3 in base 10. There are two neighbors
 * which can be numbered 0 and 1. Because 3 mod 2 = 1, the 2nd neighbor (number
 * 1) is selected. The 2nd neighbor has value 1. Therefore, the cell's new value
 * will be 1.
 * <p>
 * In two dimensions, the cells are numbered clockwise starting at the upper
 * left (see the tooltip descriptions of the lattices). For example, on a square
 * (8 neighbor) lattice, the neighbors are numbered as
 * 
 * <pre>
 *  012
 *  7x3
 ...
 * 01234567. In the one-dimensional nearest-neighbor lattice, the neighbors are
 * arranged as 0x1 and when concatenated, they are arranged as 01.
 * <p>
 * This rule is very similar to Chutes, Ladders, and Shifts. When concatenating,
 * the cell itself is not included in this rule, but it is included in Chutes,
 * Ladders, and Shifts.
 * 
 * @author David Bahr
 */
public class TunnellingSpaceships extends IntegerRuleTemplate
{
    /**
     * A display name for this class.
     */
    public static final String RULE_NAME = "Tunnelling Spaceships";

    // a tooltip description for this rule
    private static final String TOOLTIP = "<html> <body><b>"
        + RULE_NAME
        + ".</b> Produces spaceship-like lines that tunnel through each other.</body></html>";
    ...
    private static final String BEST_RESULTS = "<html> <body><b>"
        + RULE_NAME
        + ".</b>"
        + "<p> <b>For best results</b>, try a square (Margolus) 100 by 100 lattice with 31 "
        + "states and a 50% random initial state. Produces lines that tunnel through each "
        + "other. Also get spaceships with the number of states set to 4, 7, 10, 34, and others."
        + "<p>"
        + "The behavior changes dramatically with the "
        + "number of states, but is generally more interesting for large numbers "
        + "of states. "
        + "<p> "
        ...
        + "Ditto with the number of states set to 3, 13, 15, 23, 29, and other values "
        + "(experiment!). "
        + "<p>"
        ...
        + "create alternate symbols for concatenation." + leftClickInstructions
     ...
     */
    public TunnellingSpaceships(boolean minimalOrLazyInitialization)
    {
    ...
    protected int integerRule(int cellValue, int[] neighbors, int numStates,
        int generation)
    {
        // concatenate the neighbors as a base-numStates number
        ...
        BigInteger numberOfNeighbors = new BigInteger("" + neighbors.length);
        ...
        int returnValue = neighbors[neighborPosition];
