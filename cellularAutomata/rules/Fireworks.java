/*
 Fireworks -- a class within the Cellular Automaton Explorer. 
...
/**
 * A fun rule that looks like fireworks when started from a single seed. An
 * extension of life-like rules where even numbers of neighbors cause births (up
 * to 200; i.e., 0, 2, ..., 200). Odd numbers cause death (up to 201; i.e., 1,
 * 3, 5, ..., 201).
 * 
 * @author David Bahr
 */
public class Fireworks extends LifeExtensionsTemplate
{
    // The number of states for this rule
    private static final int NUMBER_OF_STATES = 50;

    // a display name for this class
    private static final String RULE_NAME = "Fireworks";
    ...
    private static final String BEST_RESULTS = "<html> <body><b>"
        ...
        + ".</b>"
        + "<p> "
        + "<b>For best results</b>, try starting from a single seed on a 50 by 50 grid, and "
        + "use the \"Fire\" color scheme.  Give the simulation a minute to become "
        + "\"explosive\".  Using a small 1% random initial state also works well. "
        + "<p>"
    ...
    private String TOOLTIP = "<html> <body><b>" + RULE_NAME
        + ".</b> Creates fireworks from single seeds.</body></html>";
     ...
     */
    public Fireworks(boolean minimalOrLazyInitialization)
    {
    ...
    protected int[] getSurvivalValues()
    {
        int[] survivalValues = new int[200];
        for(int i = 0; i < survivalValues.length; i++)
        {
            survivalValues[i] = 2 * i + 1;
        }
