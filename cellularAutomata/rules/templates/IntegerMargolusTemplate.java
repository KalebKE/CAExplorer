/*
 IntegerMargolusTemplate -- a class within the Cellular Automaton Explorer. 
...
/**
 * A convenience class/template for all rules that use the Margolus
 * neighborhood with integer states. This class handles conversion of the
 * neighbors as Cells to neighbors as integer values so that the subclass only
 * has to worry about specifying the integer function. This conversion makes
 * sure that the cells all return their state values for the same generation as
 * the current cell.
 * <p>
 ...
 * @author David Bahr
 */
public abstract class IntegerMargolusTemplate extends IntegerRule
{
	 ...
	 */
	public IntegerMargolusTemplate(boolean minimalOrLazyInitialization)
	{
		...
		if(block == null)
		{
			if(arrayPosition == MargolusLattice.NORTHWEST)
			{
				block = blockRule(cellValue, neighborValues[0],
						neighborValues[1], neighborValues[2], numStates,
						generation);
			...
			else if(arrayPosition == MargolusLattice.SOUTHEAST)
			{
				block = blockRule(neighborValues[0], neighborValues[1],
						cellValue, neighborValues[2], numStates, generation);
			...
			{
				block = blockRule(neighborValues[0], neighborValues[1],
						neighborValues[2], cellValue, numStates, generation);
	 ...
	 */
	protected abstract int[] blockRule(int northWestCellValue,
			int northEastCellValue, int southEastCellValue,
			int southWestCellValue, int numStates, int generation);
}
