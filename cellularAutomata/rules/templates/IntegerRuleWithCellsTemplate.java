/*
 IntegerRuleWithCellsTemplate -- a class within the Cellular Automaton Explorer. 
...
/**
 * Almost identical to the IntegerRuleTemplate class (see that description).
 * However, in this case, the abstract integerRule() method also passes in the
 * cell and neighbors. Sometimes a rule needs the actual cells to extract
 * additional information (like the coordinates of the cell).
 * <p>
 * For an example that uses this class, see the IsingModel rule.
 * 
 * @author David Bahr
 */
public abstract class IntegerRuleWithCellsTemplate extends IntegerRule
{
	/**
	 ...
	 */
	public IntegerRuleWithCellsTemplate(boolean minimalOrLazyInitialization)
	{
	 ...
	 *            constructed. If uncertain, set this variable to false.
	 */
	public IntegerRuleWithCellsTemplate(int numStates,
	...
	private boolean checkNum(int state)
	{
		...
		if(numStates < MIN_NUM_STATES)
		{
			throw new IllegalArgumentException(
					"Class: IntegerRuleWithCellsTemplate. Method: checkNum. The max "
							+ "number of states must be greater than "
							+ (MIN_NUM_STATES - 1) + ".");
		}
	...
	protected abstract int integerRule(int cellValue, int[] neighbors,
			int numStates, int generation, Cell cell, Cell[] neighborsCells);
}
