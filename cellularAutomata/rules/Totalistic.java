/*
 Totalistic -- a rule class within the Cellular Automaton Explorer. 
...
/**
 * Creates a totalistic CA rule for a given lattice and rule "code".
 * 
 * @author David Bahr
 */
public class Totalistic extends IntegerRuleTemplate
{
	/**
	 ...
	 */
	public static final String RULE_NAME = "Totalistic";
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ "this leads to "
			+ "snowflake-like patterns. For example, try the hexagonal (6 neighbor) "
			+ "lattice with 200 by 200 cells, rule 90, and 2 states. "
			+ "<p>"
			+ "Also try a one-dimensional (2 neighbor) lattice with 600 by 600 cells, "
			+ "random initial conditions, 3 states, and  rule 2043.  Other rules such "
			+ "as 2007, 1659, and 1029 (among many others!) also give interesting "
			+ "results. " + leftClickInstructions + rightClickInstructions
			+ "</body></html>";

	// a tooltip description for this class
	private String TOOLTIP = "<html> <body><b>"
			+ RULE_NAME
			+ ".</b> Beautiful symmetric rules that depend only on the sum of the "
			+ "neighbors and the cell itself.</body></html>";
	 ...
	 */
	public Totalistic(boolean minimalOrLazyInitialization)
	{
	...
	private void createLookUpTable()
	{
		...
		// the maximum total that can be achieved by adding together the cell
		// and its neighbor's values. (Note, we add one so that include the
		// neighbors *plus* the cell, so totalistic)
		int maxTotal = (numberOfNeighbors + 1) * (numStates - 1);

		// create an array to hold the rule (add one because the possibilities
		// go from 0 to maxTotal, not 1 to maxTotal)
		rule = new int[maxTotal + 1];
	...
	/**
	 * Sets instance variable read from the properties object.
	 */
	...
	{
		BigInteger maxRuleNumber = BigInteger.ZERO;

		try
		{
			// maxSummationValue is the max sum that can be achieved by adding
			// together all of the neighbors (including the cell itself)
			int maxSummationValue = (numberOfNeighbors + 1) * (numStates - 1);
			...
			boolean sizeOk = checkSizeOfMaxRule(numStates, maxSummationValue);

			if(sizeOk)
			{
				// the total number of rules (for each possible total there are
				// numStates possible values that can be assigned to the cell).
				BigInteger numberOfStates = BigInteger.valueOf(numStates);
				maxRuleNumber = numberOfStates.pow(maxSummationValue);

				// subtract 1 because we start counting rules from 0
				maxRuleNumber = maxRuleNumber.subtract(BigInteger.ONE);
			}
			else
			{
				// get a more "reasonable" max rule number
				BigInteger ten = BigInteger.valueOf(10);
				maxRuleNumber = ten.pow(IntegerRule.maxRuleSize + 1).subtract(
						BigInteger.ONE);
			}
		}
		catch(Throwable t)
		{
			...
			maxRuleNumber = BigInteger.valueOf(Long.MAX_VALUE);
		}
	...
	/**
	 * Updates the cell based on the totalistic rule number ("code") provided by
	 * the constructor.
	 * 
	 * @param cell
			...
			String latticeDescription, int numStates)
	{
		...
		BigInteger maxRule = getMaxRuleNumber(numStates, TwoDimensionalLattice
				.getNumberOfNeighbors(latticeDescription));
