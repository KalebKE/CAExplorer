/*
 RandomUpdate -- a class within the Cellular Automaton Explorer. 
...
/**
 * Randomly chooses a new state for the cell, independent of the neighbors.
 * 
 * @author David Bahr
 */
public class RandomUpdate extends IntegerRuleTemplate
{
    // a display name for this class
    private static final String RULE_NAME = "Random Update";
    ...
    private static final String BEST_RESULTS = "<html> <body><b>"
        ...
        + ".</b>"
        + "<p> "
        + "<b>For best results</b>, choose any number of states. No matter what "
        + "initial state configuration that you specify, the rule "
        + "randomly picks a new state for each cell (from 0 to the number of "
        + "states you specified), so all you will ever get "
        + "is uniform noise." + leftClickInstructions + rightClickInstructions
        + "</body></html>";

    // a tooltip description for this class
    private String TOOLTIP = "<html> <body><b>" + RULE_NAME
        + ".</b> Randomly selects arbitrary values at "
        + "each time step.</body></html>";

    private static Random random = RandomSingleton.getInstance();

    /**
     * Create Random Update rule using the given cellular automaton properties.
	 * <p>
     ...
     */
    public RandomUpdate(boolean minimalOrLazyInitialization)
    {
    ...
    protected int integerRule(int cell, int[] neighbors, int numStates,
        int generation)
    {
        // int randomState = random.nextInt(2);
        //
        // return (cell+randomState) % numStates;

        int randomState = random.nextInt(numStates);
