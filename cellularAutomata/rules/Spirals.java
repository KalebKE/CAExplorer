/*
 Spirals -- a class within the Cellular Automaton Explorer. 
 ...
 * unwinding. But the behavior varies dramatically with the lattice. Most other
 * lattices are boring, but try the hexagonal lattice which produces stunning,
 * drunk gliders. The lattice is based on a Life-like rule.
 * <p>
...
public class Spirals extends LifeExtensionsTemplate
{
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ ".</b>"
			+ "<p> "
			+ "<b>For best results</b>, start with a 10% random initial state and a "
			+ "square (8 neighbor) lattice.  To see the spirals, the simulation may "
			+ "need to run for hundreds of generations. To draw specific patterns, "
			+ "select the \"1\" state from the color chooser, and use this color to draw "
			+ "patterns.  All other colors quickly die away. "
			+ "<p>"
			+ "Also try a hexagonal lattice with a 50% random initial state.  This produces "
	...
	private String TOOLTIP = "<html> <body><b>" + RULE_NAME
			+ ".</b> Unwinding spirals and drunk gliders.</body></html>";

	/**
	 * Create spirals from a Life-like rule.
	 * <p>
	 ...
	 */
	public Spirals(boolean minimalOrLazyInitialization)
	{
	 ...
	 * @author David Bahr
	 */
	private class FilledView extends IntegerCellStateView
	{
		/**
		 ...
		 *      cellularAutomata.cellState.model.CellState)
		 */
