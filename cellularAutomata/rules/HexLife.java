/*
 HexLife -- a class within the Cellular Automaton Explorer. 
...
import cellularAutomata.cellState.view.HexagonalIntegerCellStateView;
import cellularAutomata.lattice.HexagonalLattice;
import cellularAutomata.rules.util.RuleFolderNames;

/**
 * Same as the class Life, but with hexagonal graphics. The nicer graphics slow
 ...
 * @author David Bahr
 */
public class HexLife extends Life
{
    // a display name for this class
    private static final String RULE_NAME = "Hexagonal Life";
    ...
    private static final String BEST_RESULTS = "<html> <body><b>"
        ...
        + ".</b>"
        + "<p> "
        + "<b>For best results</b>, use a 50% random initial state.  Look for a much "
        + "wider variety of stable structures and some occasional blinkers, but don't "
        + "expect much. On grids with fewer than 8 neighbors, the Game of Life suffers "
    ...
    private String TOOLTIP = "<html> <body><b>" + RULE_NAME
        + ".</b> Same as the \"Life\" rule, but uses nicer hexagonal "
     ...
     */
    public HexLife(boolean minimalOrLazyInitialization)
    {
	...
	{
		return new HexagonalIntegerCellStateView();
	}
