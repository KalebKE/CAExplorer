/*
 FractalThreshold -- a class within the Cellular Automaton Explorer. 
...
/**
 * A rule which assigns complex numbers to each cell; if the modulus of "the sum
 * of the neighbors minus the cell" is less than a threshold, then the cell is
 * multiplied by 1-i. The results are displayed as Mandelbrot and Julia sets.
 * 
 * @author David Bahr
 */
public class FractalThreshold extends ComplexRuleTemplate
{
    ...
    private static final double DEFAULT_JULIA_REAL_VALUE = -0.9;
    ...
    private static final double DEFAULT_WIDTH = 7.0;
    ...
    private static final String RULE_NAME = "Fractal Threshold";
    ...
    private static final String BEST_RESULTS = "<html> <body><b>"
        ...
        + "Mandelbrot Set view. Center at the "
        + "position -0.2 + 0.0i with width 7.0.  Once you "
        + "have tried this configuration, try changing various parameters for other incredible "
        + "results. For example, try using the Julia Set view, and try various values for c with "
        + "the Julia Set view.  Also try using a large square or circular initial state."
        + "<p>"
    ...
    private String TOOLTIP = "<html> <body><b>"
        + RULE_NAME
        + ".</b> Generates a steady-state fractal pattern from a complex-valued rule. </body></html>";
    ...
    /**
     * Create a rule that displays fractals based on a threshold.
     * <p>
     ...
     */
    public FractalThreshold(boolean minimalOrLazyInitialization)
    {
        ...
        Complex[] neighborValues, int generation)
    {
        ...
        Complex neighborSum = new Complex(0.0, 0.0);
        for(Complex c : neighborValues)
        {
            neighborSum = Complex.add(neighborSum, c);
        }

        // also try 1.0 and other constants in place of threshold = 10.0
        // And try && Complex.minus(neighborSum, newValue).modulus() >
        // threshold2) with threshold2 = 5.0;
        double threshold = 10.0;
        if(Complex.minus(neighborSum, newValue).modulus() < threshold)
        {
            newValue = Complex.multiply(newValue, new Complex(1.0, -1.0));
            if(newValue.modulus() > getFullState().modulus())
            {
                newValue = new Complex(0, 0);
            }
        }
    ...
    private class ComplexView extends TriangleHexagonCellStateView
    {
            ...
            Coordinate rowAndCol)
        {
            ...
            if(viewChoice == MANDELBROT)
            {
                ...
                // z = Complex.plus(Complex.multiply(Complex.multiply(z, z),
