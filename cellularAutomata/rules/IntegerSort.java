/*
 IntegerSort -- a class within the Cellular Automaton Explorer. 
...
import cellularAutomata.rules.templates.OneDimensionalIntegerMargolusTemplate;

/**
 * Sorts integer numbers on a one-dimensional line.
 * 
 * @author David Bahr
 */
public class IntegerSort extends OneDimensionalIntegerMargolusTemplate
{
	// a display name for this class
	private static final String RULE_NAME = "Sort Integers";
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ ".</b>"
			+ "<p> "
			+ "<b>For best results</b>, choose 200 states and a 99% random initial state "
			+ "on a 200 by 200 lattice. "
			+ "Large numbers of states on large lattices work best."
	...
	private String TOOLTIP = "<html> <body><b>" + RULE_NAME
			+ ".</b> Sorts numbers using a super fast O(N) parallel sorting "
			+ "algorithm.</body></html>";

	/**
	 * Sorts numbers by swapping adjacent pairs as necessary. An O(N^2)
	 ...
	 */
	public IntegerSort(boolean minimalOrLazyInitialization)
	{
	 ...
	 *         side of the block.
	 */
	protected int[] blockRule(int westCellValue, int eastCellValue,
			int westCellsFixedColumnPosition, int numStates, int generation)
	{
		int[] newBlockValues = new int[2];
