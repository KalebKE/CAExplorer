/*
 MinorityWins -- a class within the Cellular Automaton Explorer. 
...
/**
 * Rule for the minority winning. In other words, if a cell is surrounded by a
 * majority of a given type, then the cell becomes the opposite of that type.
 * 
 * @author David Bahr
 */
public class MinorityWins extends IntegerRuleTemplate
{
    // a display name for this class
    private static final String RULE_NAME = "Minority Wins";
    ...
    private static final String BEST_RESULTS = "<html> <body><b>"
        ...
        + "on a square (8 neighbor) lattice.  Try varying the initial population from 45% "
        + "to 50% to 55% to see changes in behavior. Also try making a 3 party system by "
        + "using 3 states with a 66% initial population.  And then try 4 states with 75%, and "
        + "5 states with 80%, etc. "
        + "<p>"
        + "To avoid a headache when using two-dimensional grids, try using a running average of "
        + "2 time steps.  The average boundary behavior is fascinating near "
        + "the phase change (50%).  Also try averaging 2, 3, or 4 time steps "
        + "with 3 states and a 66% initial population.  The physicists and mathematical "
        + "sociologists out there will think this is way cool!  There is some neat behavior being "
        + "delineated by this averaging." + leftClickInstructions
        + rightClickInstructions + "</body></html>";

    // a tooltip description for this class
    private String TOOLTIP = "<html> <body><b>"
        + RULE_NAME
        + ".</b> A rebellious model of decision making where a person always chooses to "
        + "go with the minority opinion.</body></html>";

    private static Random random = RandomSingleton.getInstance();

    /**
     * Create the Minority rule using the given cellular automaton properties.
	 * <p>
     ...
     */
    public MinorityWins(boolean minimalOrLazyInitialization)
    {
        super(minimalOrLazyInitialization);
    }

    /**
     * Rules for minority wins (excluding the cell itself).
     * 
     * @param cell
        ...
        int generation)
    {
        ...
        for(int i = 0; i < numStates; i++)
        {
            if(numberOfEachState[i] < numberOfEachState[minState])
            {
            ...
            else if(numberOfEachState[i] == numberOfEachState[minState])
            {
        ...
        minState = allTheStatesWithMin.get(
            random.nextInt(allTheStatesWithMin.size())).intValue();
