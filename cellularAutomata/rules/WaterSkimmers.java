/*
 WaterSkimmers -- a class within the Cellular Automaton Explorer. 
...
/**
 * Takes the average of the cell and neighbors, adds a small random amount, and
 * modulos by the maximum value.
 * 
 * @author David Bahr
 */
public class WaterSkimmers extends RealRuleTemplate
{
	...
	private static final String RULE_NAME = "Water Skimmers";
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ ".</b>"
			+ "<p> "
			+ "<b>For best results</b>, try a random initial population on a 200 by 200 "
			+ "lattice. Be patient when first starting the simulation. The skimmers will "
			+ "appear after a while.  On small lattices, the visuals are best when run "
			+ "slowly, so try increasing the delay on the Control Panel."
	...
	private String TOOLTIP = "<html> <body><b>" + RULE_NAME
			+ ".</b> Water-skimming bugs on a puddle.</body></html>";

	/**
	 * Create a rule that takes the average of the cell and neighbors, adds a
	 * small random amount, and modulos by the maximum value.
	 * <p>
	 ...
	 */
	public WaterSkimmers(boolean minimalOrLazyInitialization)
	{
		super(MIN_VALUE, MAX_VALUE, minimalOrLazyInitialization);
	}

	/**
	 * Takes the average of the cell and neighbors, adds a small random amount,
	 * and modulos by the maximum value.
	 * 
	...
	public double doubleRule(double cell, double[] neighbors, int generation)
	{
		...
		// take the average
		sum /= (neighbors.length + 1);

		// now add a token amount
		sum += 10.0 * RandomSingleton.getInstance().nextDouble();

		// "modulo" the max value
		if(sum > MAX_VALUE)
		{
			sum = ((sum / MAX_VALUE) - Math.floor(sum / MAX_VALUE)) * MAX_VALUE;
		}
