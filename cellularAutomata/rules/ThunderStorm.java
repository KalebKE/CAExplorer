/*
 ThunderStorm -- a class within the Cellular Automaton Explorer. 
...
/**
 * Takes the average of the cell and neighbors, adds a small periodic amount,
 * and modulos by the maximum value.
 * 
 * @author David Bahr
 */
public class ThunderStorm extends RealRuleTemplate
{
	...
	private static final String RULE_NAME = "Thunder Storm";
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ "square (8 neighbor) lattice. Also interesting on a one-dimensional lattice. "
			+ "Best when run slowly, so try increasing the delay on the Control Panel."
	...
	private String TOOLTIP = "<html> <body><b>"
			+ RULE_NAME
			+ ".</b> Periodic lightning storms with rain drops on a puddle.</body></html>";

	/**
	 * Create a rule that takes the average of the cell and neighbors, adds a
	 * periodic amount, and modulos by the maximum value.
	 * <p>
	 ...
	 */
	public ThunderStorm(boolean minimalOrLazyInitialization)
	{
	...
	public double doubleRule(double cell, double[] neighbors, int generation)
	{
		...
		sum += 25.0 * (1.0 + Math.sin(generation / 10.0));
