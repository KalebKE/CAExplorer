/*
 AlternateContinuousCA -- a class within the Cellular Automaton Explorer. 
...
/**
 * Takes the average of the neighbors, plugs into the linear equation "slope *
 * avg + yInterecept", and then keeps only the fractional part. Very similar to
 * ContinuousCA, but this class does not include the cell when calculating the
 * average.
 * 
 * @author David Bahr
 */
public class AlternateContinuousCA extends RealRuleTemplate
{
	// default slope value for the linear equation that calculates the
	// cell's new value
	private static final double DEFAULT_SLOPE_VALUE = 1.40;

	// default y-intercept value for the linear equation that calculates the
	// cell's new value
	private static final double DEFAULT_Y_INTERCEPT_VALUE = 0.99;
	...
	private static final String RULE_NAME = "Alternate Continuous CA";
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ ".</b>"
			+ "<p> "
			+ "<b>For best results</b>, try a single seed or random initial state on a 200 by "
			...
			+ "change (open the More Properties box)."
			+ "<p> Starting from a single seed in two dimensions can give fantastic "
			+ "kaleidoscope behaviors." + leftClickInstructions
	...
	private String TOOLTIP = "<html> <body><b>"
			+ RULE_NAME
			+ ".</b> Creates diamonds similar to the Reversible rules, but uses "
			+ "real numbers.</body></html>";

	/**
	 * Takes the average of the neighbors, plugs into the linear equation "slope *
	 * avg + yInterecept", and then keeps only the fractional part.
	 * <p>
	 ...
	 */
	public AlternateContinuousCA(boolean minimalOrLazyInitialization)
	{
	...
	private JPanel createDescriptionPanel()
	{
		...
		String functionDescription = "These controls let you set the slope and y-intercept "
				...
				+ "neighbors.  The cell is assigned the fractional part of y. "
				+ "(In other words, the whole number part is discarded.) \n\n"
				+ "With a one-dim (2 neighbor) lattice, try random initial states with "
				+ "values such as \n\n"
	 ...
	 */
	private JPanel createSlopeAndInterceptSpinners()
	{
	 ...
	 *      double, double[], int)
	 */
	public synchronized double doubleRule(double cell, double[] neighbors, int generation)
	{
		...
		// average the neighbors (but not the cell)
		double avg = 0.0;
	...
	public JPanel getAdditionalPropertiesPanel()
	{
		...
		if(panel == null)
		{
			panel = createSlopeAndInterceptSpinners();
		}
