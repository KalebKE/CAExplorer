/*
 EpilepticBlobs -- a class within the Cellular Automaton Explorer. 
...
/**
 * Outer totalistic rule 429.
 * 
 * @author David Bahr
 */
public class EpilepticBlobs extends OuterTotalistic
{
	// The number of states for this rule
	private static final int NUMBER_OF_STATES = 2;

	// The outer totalistic rule number
	private static int OUTER_TOTALISTIC_RULE_NUMBER = 429;

	// a display name for this class
	private static final String RULE_NAME = "Epileptic Blobs";
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ ".</b>"
			+ "<p> "
			+ "<b>For best results</b>, start from a 50% random initial state "
			+ "with a 150 by 150 square (8 neighbor) or hexagonal (6 neighbor) lattice. "
			+ "Try the Fire color scheme for good contrast.  <br><br>"
			+ "A running average "
			+ "of 2 will eliminate the flashing and create fascinating loops "
			+ "(in this case, it is equivalent to the Super Loop rule).<br><br>"
			+ "To see the flashing, you may need to slow the speed of the "
			+ "simulation." + leftClickInstructions + rightClickInstructions
			+ "</body></html>";

	// a tooltip description for this class
	private String TOOLTIP = "<html> <body><b>"
			+ RULE_NAME
			+ ".</b> Seizure-inducing blobs of alternating color.</body></html>";

	/**
	 * Create outer totalistic rule 429.
	 * <p>
	 ...
	 */
	public EpilepticBlobs(boolean minimalOrLazyInitialization)
	{
