...
import userRules.cellularMarketModel.math.AutoCorrelation;
 ...
 * 
 */
public class PricingAutoCorrAnalysis extends Analysis implements ActionListener
{
	...
	private static final String ANALYSIS_NAME = "Pricing Auto Corr Analysis";
	 ...
	 */
	public PricingAutoCorrAnalysis(boolean minimalOrLazyInitialization)
	{
	...
	private void constructAnalysis()
	{
		...
		if (isCompatibleRule)
		{
			...
			if (numberOfStates <= MAX_NUMBER_OF_STATES)
			{
				...
				// this workaround :-(
				timeSeriesOfVolatilityList = new LinkedList[numberOfStates];
				for (int i = 0; i < timeSeriesOfVolatilityList.length; i++)
				{
					timeSeriesOfVolatilityList[i] = new LinkedList<Point2D.Double>();
					Point2D.Double point = new Point2D.Double();
					point.setLocation(0, 0);
					timeSeriesOfVolatilityList[i].add(point);
				}
	...
	private void createDisplayPanel()
	{
		...
		if (isCompatibleRule && (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			plot = new SimplePlot();
			timeSeriesPlot = new PricingVolatilityPlot();
	...
	private JPanel createMessagePanel()
	{
		...
		AttentionPanel attentionPanel = new AttentionPanel("Absolute Autocorrelation");
	...
	private void plotTimeSeriesData()
	{
		...
		for (int i = startPosition; i < timeSeriesOfVolatilityList.length; i++)
		{
			allPoints.addAll(timeSeriesOfVolatilityList[i]);
		}

		// set the min and max values on the plot
		Point2D firstPoint = (Point2D) timeSeriesOfVolatilityList[0].getFirst();
		...
		for (int state = startPosition; state < timeSeriesOfVolatilityList.length; state++)
		{
			Color stateColor = view.getDisplayColor(
					new IntegerCellState(state), null, new Coordinate(0, 0));

			int listLength = timeSeriesOfVolatilityList[state].size();
	...
	protected void analyze(Lattice lattice, Rule rule, int generation)
	{
				...
				&& (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			try
			{
				double[][] absLogReturn = LogReturn
						.calcABSLogReturn(historicalPricing);

				int lag = 1;

				double historicalVolatility[][] = new double[numberOfStates][];

				for (int i = 0; i < numberOfStates; i++)
				{
					historicalVolatility[i] = AutoCorrelation.autoCorrelation(
							absLogReturn[i], lag);
				}
				...
				for (int i = 0; i < historicalVolatility.length; i++)
				{
					...
					{
						timeSeriesOfVolatilityList[i]
								.removeAll(timeSeriesOfVolatilityList[i]);
						for (int j = 0; j < historicalVolatility[i].length; j++)
						{
							timeSeriesOfVolatilityList[i]
									.add(new Point2D.Double(j,
											historicalVolatility[i][j]));
							if (timeSeriesOfVolatilityList[i].size() > MAX_NUMBER_TO_PLOT)
							{
								timeSeriesOfVolatilityList[i].removeFirst();
							}
						}
					}
				}
			...
			for (int i = 0; i < marketValue.length; i++)
			{
				String output = percentFormatter
						.format(timeSeriesOfVolatilityList[i].getLast().getY());
