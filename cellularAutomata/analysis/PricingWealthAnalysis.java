...
import cellularAutomata.util.WealthPlot;
 ...
 * 
 */
public class PricingWealthAnalysis extends Analysis implements ActionListener
{
	private static final double DEFAULT_WEALTH = 1000;

	// the maximum number of elements that will be plotted on the time series
	// plot
	private static final int MAX_NUMBER_TO_PLOT = 50;
	...
	private static final String ANALYSIS_NAME = "Pricing Wealth Analysis";
	...
	// keep track of the wealth for every cell
	private LinkedList<Point2D.Double>[] timeSeriesOfWealth = null;

	private LinkedList<Integer> cellPrevState = null;

	private LinkedList<Double> buyPrice = null;

	// a panel that plots the pricing data
	private SimplePlot plot = null;

	// a panel that plots the pricing data as a time series
	private WealthPlot timeSeriesPlot = null;
	 ...
	 */
	public PricingWealthAnalysis(boolean minimalOrLazyInitialization)
	{
	...
	private void constructAnalysis()
	{
		...
		if (isCompatibleRule)
		{
			...
			numCells = CurrentProperties.getInstance().getNumColumns()
					* CurrentProperties.getInstance().getNumRows();
			try
			{
				if (numberOfStates <= MAX_NUMBER_OF_STATES)
				{
					...
					// note that you cannot create an array of generics without
					// this workaround :-(
					// create a really, really massive data structure to
					// store the time series of each cells wealth.
					timeSeriesOfWealth = new LinkedList[numCells];

					for (int i = 0; i < timeSeriesOfWealth.length; i++)
					{
						timeSeriesOfWealth[i] = new LinkedList<Point2D.Double>();

						timeSeriesOfWealth[i].add(new Point2D.Double(0, DEFAULT_WEALTH));
					}

					cellPrevState = new LinkedList<Integer>();
					buyPrice = new LinkedList<Double>();
	...
	private void createDisplayPanel()
	{
		...
		if (isCompatibleRule && (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			JLabel percentOccupiedLabel = new JLabel("Price:");
	...
	private JPanel createMessagePanel()
	{
		...
		AttentionPanel attentionPanel = new AttentionPanel("Wealth Analysis");
	...
	private void plotTimeSeriesData()
	{
		...
		for (int i = startPosition; i < timeSeriesOfWealth.length; i++)
		{
			allPoints.addAll(timeSeriesOfWealth[i]);
		}

		// set the min and max values on the plot
		Point2D firstPoint = (Point2D) timeSeriesOfWealth[0].getFirst();
		...
		timeSeriesPlot.setYAxisLabel("cell wealth");
		...
		int numberOfInteriorYValueLabels = (int) ((maxYValue / 200));
		double[] yValues = new double[numberOfInteriorYValueLabels];
		for (int i = 0; i < yValues.length; i++)
		{
			double answer = (i * 200);
		...
		for (int state = startPosition; state < timeSeriesOfWealth.length; state++)
		{
			Color stateColor = view.getDisplayColor(
					new IntegerCellState(state%numberOfStates), null, new Coordinate(0, 0));

			int listLength = timeSeriesOfWealth[state].size();
	...
	protected void analyze(Lattice lattice, Rule rule, int generation)
	{
				...
				&& (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			while (cellIterator.hasNext())
			{
				...
				// if the cell sold and purchased a new asset, record it
				if (state.toInt() != cellPrevState.get(totalNumberOfCells))
				{
					// get the sale price of the asset that was sold (the cells
					// previous state)
					double salePrice = priceList.get(
							cellPrevState.get(totalNumberOfCells)).getY();

					// calculate the profit by subtracting the price the asset
					// was originally purchased at from the sale price.
					profit = salePrice - this.buyPrice.get(totalNumberOfCells);
					
					// calculate the net profit on the old asset
					net = profit + this.buyPrice.get(totalNumberOfCells);

					// update the cells state
					cellPrevState.set(totalNumberOfCells, state.toInt());
					
					// update the buy price
					this.buyPrice.set(totalNumberOfCells, marketValue[state.toInt()]);
					
					sale = true;
				}

				// Get the previous wealth of the cell
				double previousWealth = timeSeriesOfWealth[totalNumberOfCells]
						.getLast().getY();

				// Calculate the new wealth the cell
				// previous wealth + (net - the cost of the new asset)
				if(sale)
				{
				timeSeriesOfWealth[totalNumberOfCells].add(new Point2D.Double(
						generation, previousWealth
								+ (net - marketValue[state.toInt()])));
				}
				// just the previous wealth if no asset sale
				else
				{
					timeSeriesOfWealth[totalNumberOfCells].add(new Point2D.Double(
							generation, previousWealth));
				...
				if(timeSeriesOfWealth[totalNumberOfCells].getLast().getY() > maxWealth)
				{
					maxWealth = timeSeriesOfWealth[totalNumberOfCells].getLast().getY();
					System.out.println(maxWealth);
					System.out.println(lattice.getNeighbors(cell).length);
				}
			...
			for (int cellIndex = 0; cellIndex < timeSeriesOfWealth.length; cellIndex++)
			{
				if (timeSeriesOfWealth[cellIndex].size() > MAX_NUMBER_TO_PLOT)
				{
					timeSeriesOfWealth[cellIndex].removeFirst();
				}
			}
			...
			for (int i = 0; i < marketValue.length; i++)
			{
				String output = percentFormatter.format(marketValue[i]);
