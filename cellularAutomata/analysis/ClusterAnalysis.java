/*
 ClusterAnalysis -- a class within the Cellular Automaton Explorer. 
...
/**
 * Finds the size and distributions of clusters of states.
 * 
 * @author David Bahr
 */
public class ClusterAnalysis extends Analysis implements ActionListener
{
	...
	private static final String ANALYSIS_NAME = "Cluster Sizes";

	// display info for this class
	private static final String INFO_MESSAGE = "Neighboring cells with the "
			+ "same state are in the same cluster. In other words, if two cells with the "
			+ "same state are connected by a chain of other cells with the same state "
			+ "then they belong to the same cluster. This analysis finds the number of "
			+ "separate clusters and their sizes.";

	// the label for the "plot zero values" check box
	private static final String PLOT_ZERO_VALUES = "   Plot clusters of size 0";
	...
	private static final String SAVE_DATA_TOOLTIP = "<html>Saves cluster data "
	...
	private static final String TOOLTIP = "<html>counts clusters of connected cells "
			+ "that have the same state</html>";
	...
	// the number of clusters of each size
	private int[] numberOfClustersOfEachSize = null;
	...
	// the cluster data as points of (size, number of that size)
	private Point2D.Double[] clusterNumbers = null;
	...
	/**
	 * Create an analyzer that counts cluster sizes.
	 * <p>
	 ...
	 */
	public ClusterAnalysis(boolean minimalOrLazyInitialization)
	{
	...
	/**
	 * Converts the cluster sizes to a Point2D array for plotting (not relevant
	 * to the log-log plot -- this is only used by the regular plot).
	 */
	private void convertClusterSizesToPointArray()
	{
		synchronized(this)
		{
			if(plotZeroValues)
			{
				...
				if(numberOfClustersOfEachSize.length > 0)
				{
					clusterNumbers = new Point2D.Double[numberOfClustersOfEachSize.length];
					for(int i = 0; i < clusterNumbers.length; i++)
					{
						// add 1 so not plotting clusters of size 1 on position
						// 0
						clusterNumbers[i] = new Point2D.Double(i + 1,
								numberOfClustersOfEachSize[i]);
					}
				}
				else
				{
					clusterNumbers = new Point2D.Double[1];
					clusterNumbers[0] = new Point2D.Double(0, 0);
				}
			}
			else
			{
				...
				if(numberOfClustersOfEachSize.length > 0)
				{
					...
					for(int i = 0; i < numberOfClustersOfEachSize.length; i++)
					{
						if(numberOfClustersOfEachSize[i] > 0)
						{
							numList.add(new Point2D.Double(i + 1,
									numberOfClustersOfEachSize[i]));
						}
					}
					...
					// written :-(
					clusterNumbers = new Point2D.Double[numList.size()];
					for(int i = 0; i < clusterNumbers.length; i++)
					{
						clusterNumbers[i] = numList.get(i);
					}
				}
				else
				{
	...
	private void createDisplayPanel()
	{
		...
		displayPanel.add(plotZeroValuePanel, new GBC(1, row).setSpan(4, 1)
				.setFill(GBC.NONE).setWeight(10.0, 10.0).setAnchor(GBC.CENTER)
				.setInsets(1));
	...
	private JPanel createMessagePanel()
	{
		...
		AttentionPanel attentionPanel = new AttentionPanel("Cluster Sizes");
	...
	/**
	 * Plots the cluster data on a regular plot.
	 */
	private void plotData()
	{
		// axes labels
		plot.setXAxisLabel("cluster size, N");
		plot.setYAxisLabel("# of clusters, S(N)");
		...
		for(int i = 0; i < numberOfClustersOfEachSize.length; i++)
		{
			if(numberOfClustersOfEachSize[i] > maxYValue)
			{
				maxYValue = numberOfClustersOfEachSize[i];
			}
		}
		if(maxYValue < 1)
		{
		...
		for(int i = 0; i < clusterNumbers.length; i++)
		{
			if(clusterNumbers[i].x > maxXValue)
			{
				maxXValue = (int) clusterNumbers[i].x;
			}
		}
		...
		if(clusterNumbers.length > 0)
		{
			...
			Color[] colorArray = new Color[clusterNumbers.length];
		...
		plot.drawPoints(clusterNumbers);
	}
	...
	{
		// axes labels
		logPlot.setXAxisLabel("log(N)");
		logPlot.setYAxisLabel("log(S(N))");
		...
		for(int i = 0; i < numberOfClustersOfEachSize.length; i++)
		{
			if(numberOfClustersOfEachSize[i] > 0)
			{
				logNumbers.add(new Point2D.Double(Math.log(i + 1), Math
						.log(numberOfClustersOfEachSize[i])));
			}
	...
	/**
	 * Counts and displays the number of occupied cells using the algorithm
	 * outlined in Appendix A of "Introduction to Percolation Theory, 2nd
	...
	protected void analyze(Lattice lattice, Rule rule, int generation)
	{
		...
		// create a hashtable to hold the cluster value for each cell (key)
		Hashtable<Cell, Integer> hashOfClusterValues = new Hashtable<Cell, Integer>();

		// keeps track of which cluster labels are really the same
		Hashtable<Integer, Integer> labelMap = new Hashtable<Integer, Integer>();

		// label for empty sites
		int emptyLabel = Integer.MAX_VALUE;
		labelMap.put(new Integer(emptyLabel), new Integer(emptyLabel));

		// the next available cluster label. (Must start at 0, so is incremented
		// to 1 for the first cluster. Later the largest label is used to
		// instantiate an array, and we don't want a bad value.)
		int clusterLabelCount = 0;

		// go through the lattice and mark each cell as belonging to the cluster
		// (or not)
		Cell cell = null;
		while(cellIterator.hasNext())
		{
			...
			// clusters
			if((countingEmptyCells && !cellState.isEmpty())
					|| (countingAllOccupiedCells && cellState.isEmpty())
					|| ((!countingEmptyCells && !countingAllOccupiedCells)
							&& isIntegerCellState && (cellState.toInt() != selectedState)))
			{
				// give it the label reserved for empty sites
				hashOfClusterValues.put(cell, new Integer(emptyLabel));
			}
			else
			{
				// now check neighbors
				Cell[] neighboringCells = lattice.getNeighbors(cell);
				int newClusterLabel = emptyLabel;

				// get smallest cluster label among the neighbors
				int smallestLabel = emptyLabel;
				for(Cell neighbor : neighboringCells)
				{
					if(hashOfClusterValues.containsKey(neighbor))
					{
						int clusterValue = hashOfClusterValues.get(neighbor)
								.intValue();

						if(clusterValue < smallestLabel)
						{
							smallestLabel = clusterValue;
						}
					}
				}

				// set the cluster label for this cell to the smallest
				// neighboring label
				if(smallestLabel != emptyLabel)
				{
					newClusterLabel = smallestLabel;

					// there was a neighboring cell with this smallest
					// label, so all other larger neighboring labels
					// need to be mapped to this label
					for(Cell neighbor : neighboringCells)
					{
						if(hashOfClusterValues.containsKey(neighbor))
						{
							Integer badClusterValue = hashOfClusterValues
									.get(neighbor);

							// this keeps track of label mappings and
							// maps the bad labels to the newer good
							// labels
							if(badClusterValue.intValue() != emptyLabel)
							{
								labelMap.put(badClusterValue, new Integer(
										newClusterLabel));
							}
						}
					}
				...
				{
					// if no neighbor was already labeled, then need a
					// new label (the next largest that isn't already in
					// use)
					clusterLabelCount++;
					newClusterLabel = clusterLabelCount;
				}

				// now save the cluster number
				hashOfClusterValues.put(cell, new Integer(newClusterLabel));

				// keeps track of label mappings
				labelMap.put(new Integer(newClusterLabel), new Integer(
						newClusterLabel));
			}
		}

		// now we have to go back through the cells and map bad cluster labels
		// to good cluster labels
		int maxUsedLabel = 0;
		...
		while(cellIterator.hasNext())
		{
			...
			// get the cell's cluster label
			Integer clusterLabel = hashOfClusterValues.get(cell);

			// find the correct label
			while(!labelMap.get(clusterLabel).equals(clusterLabel))
			{
				clusterLabel = labelMap.get(clusterLabel);
			}

			// set the correct label
			hashOfClusterValues.put(cell, clusterLabel);

			// while we are at it, let's keep track of the maximum label
			// (besides the empty label, which is Integer.MAX_VALUE).
			int label = clusterLabel.intValue();
			if((label != emptyLabel) && (label > maxUsedLabel))
			{
				maxUsedLabel = label;
			}
		}

		// now find the number of clusters and their sizes
		int numberOfClusters = maxUsedLabel;
		int[] clusterSizes = null;
		if(numberOfClusters > 0)
		{
			clusterSizes = new int[numberOfClusters];
			for(int i = 0; i < clusterSizes.length; i++)
			{
				clusterSizes[i] = 0;
			}

			Collection<Integer> collectionOfClusterLabels = hashOfClusterValues
					.values();
			if(collectionOfClusterLabels != null)
			{
				Iterator<Integer> labelIterator = collectionOfClusterLabels
						.iterator();
				while(labelIterator.hasNext())
				{
					int label = labelIterator.next().intValue();

					// increase the size of each cluster
					if(label != emptyLabel)
					{
						// subtract 1 because the labels start with 1
		...
		for(int i = 0; i < clusterSizes.length; i++)
		{
			if(maxClusterSize < clusterSizes[i])
			{
				maxClusterSize = clusterSizes[i];
			}
		}

		// now count the number of clusters of each size
		numberOfClustersOfEachSize = new int[maxClusterSize];
		for(int i = 0; i < numberOfClustersOfEachSize.length; i++)
		{
			numberOfClustersOfEachSize[i] = 0;
		}
		for(int i = 0; i < clusterSizes.length; i++)
		{
			if(clusterSizes[i] - 1 >= 0)
			{
				numberOfClustersOfEachSize[clusterSizes[i] - 1]++;
			}
		}

		// convert cluster sizes to a Point2D array for plotting
		convertClusterSizesToPointArray();

		// set the text for the labels
		generationDataLabel.setText("" + generation);

		// create an array of data to be saved
		data = new String[numberOfClustersOfEachSize.length + 1];
		data[0] = "" + generation;
		for(int i = 0; i < numberOfClustersOfEachSize.length; i++)
		{
			data[i + 1] = "" + numberOfClustersOfEachSize[i];
		}
