...
/**
 * Finds the percentage of each state at each generation a specified neighborhood 
 * configuration in the previous generation. Uses nearest neighbors only.
 * 
 * @author David Bahr
 * @author Kaleb Kircher
 * @author Maria Molina
 */
public class AATPreviousGen extends Analysis implements ActionListener
{
	...
	private static final String INFO_MESSAGE = "Calculates the number and percentage "
			+ "of cells that have each state with a specified neighborhood configuration" +
	...
	/**
	 * Create an analyzer that counts the number of cells of every state a specified 
	 * neighborhood configuration in the previous generation. Uses nearest neighbors only.
	 * <p>
	 ...
	 */
	public AATPreviousGen(boolean minimalOrLazyInitialization)
	{
	...
	/**
	 * Counts and displays the number of occupied cells with a specified neighborhood 
	 * configuration in the previous generation. Uses nearest neighbors only.
	 * 
	 * @param lattice
	 *            The CA lattice.
	...
	protected void analyze(Lattice lattice, Rule rule, int generation)
	{
				...
				&& (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			while (cellIterator.hasNext())
			{
				...
				{
					previousNeighboringStates[i] = neighboringCells[i].getPreviousState();	
				}
				...
				for (int i = 0; i < neighboringCells.length; i++)
				{
					if (generation > 2)
					{
						int neighborState = previousNeighboringStates[i].toInt();
						numberOfEachState[neighborState]++;
					}
				}
				
				if(numberOfEachState[1] == 0)
				{
					totalDrinkingCandidates++; 
				}
				
				// Looking for all of the cells that are in state 1 and have a 
				// nearest neighbor that is in state 1. THIS PART OF THE CODE can 
				// be changed to analyze which ever configuration one wants to see.
				if (cell.toInt() == 1 && numberOfEachState[1] == 0)
				{
