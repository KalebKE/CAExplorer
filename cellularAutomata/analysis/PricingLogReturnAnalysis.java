...
import cellularAutomata.util.PricingVolatilityPlot;
import cellularAutomata.util.Fonts;
import cellularAutomata.util.GBC;
import cellularAutomata.util.LogReturnsPlot;
 ...
 * 
 */
public class PricingLogReturnAnalysis extends Analysis implements
		ActionListener
{
	// the maximum number of elements that will be plotted on the time series
	// plot
	private static final int MAX_NUMBER_TO_PLOT = 500;
	...
	private static final String ANALYSIS_NAME = "Pricing Log Return Analysis";
	...
	private static final String INFO_MESSAGE = "Calculates the log return of assets "
	...
	private JLabel[] LogReturnDataLabel = null;
	 ...
	 */
	public PricingLogReturnAnalysis(boolean minimalOrLazyInitialization)
	{
	...
	private void constructAnalysis()
	{
		...
		if (isCompatibleRule)
		{
			...
			if (numberOfStates <= MAX_NUMBER_OF_STATES)
			{
				...
				// this workaround :-(
				timeSeriesOfLogReturnList = new LinkedList[numberOfStates];
				for (int i = 0; i < timeSeriesOfLogReturnList.length; i++)
				{
					timeSeriesOfLogReturnList[i] = new LinkedList<Point2D.Double>();
					Point2D.Double point = new Point2D.Double();
					point.setLocation(0, 0);
					timeSeriesOfLogReturnList[i].add(point);
				}
	...
	private void createDisplayPanel()
	{
		...
		if (isCompatibleRule && (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			JLabel percentOccupiedLabel = new JLabel("Log Return:");
			...
			for (int i = 0; i < numberOfStates; i++)
			{
				boxOfPercentLabels.add(LogReturnDataLabel[i]);
				boxOfPercentLabels.add(Box.createVerticalStrut(verticalSpace));
			}
			...
			plot = new SimplePlot();
			timeSeriesPlot = new LogReturnsPlot();
	...
	private JPanel createMessagePanel()
	{
		...
		AttentionPanel attentionPanel = new AttentionPanel("Log Returns");
	...
	private void plotTimeSeriesData()
	{
		...
		for (int i = startPosition; i < timeSeriesOfLogReturnList.length; i++)
		{
			allPoints.addAll(timeSeriesOfLogReturnList[i]);
		}

		// set the min and max values on the plot
		Point2D firstPoint = (Point2D) timeSeriesOfLogReturnList[0].getFirst();
		timeSeriesPlot.setMaximumXValue(firstPoint.getX() + MAX_NUMBER_TO_PLOT
				- 1);
		timeSeriesPlot.setMinimumXValue(firstPoint.getX());
		timeSeriesPlot.setMinimumYValue(-1.0);
		...
		int numberOfInteriorYValueLabels = (int) ((maxYValue / 2));
		double[] yValues = new double[numberOfInteriorYValueLabels];
		for (int i = 0; i < yValues.length; i++)
		{
			double answer = (i * 20);
		...
		for (int state = startPosition; state < timeSeriesOfLogReturnList.length; state++)
		{
			Color stateColor = view.getDisplayColor(
					new IntegerCellState(state), null, new Coordinate(0, 0));

			int listLength = timeSeriesOfLogReturnList[state].size();
				...
				&& (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			for (int state = 0; state < marketValue.length; state++)
			{
				...
				historicalPriceList[state].add(new Point2D.Double(generation,
						marketValue[state]));
				if (historicalPriceList[state].size() > 500)
				{
			...
			double historicalVolatility[][] = LogReturn
					.calcLogReturn(historicalPricing);

			try
			{
				for (int i = 0; i < historicalVolatility.length; i++)
				{
					if (historicalVolatility[i].length != 0)
					{
						timeSeriesOfLogReturnList[i]
								.add(new Point2D.Double(
										generation,
										historicalVolatility[i][historicalVolatility[i].length - 1]));
						if (timeSeriesOfLogReturnList[i].size() > MAX_NUMBER_TO_PLOT)
						{
							timeSeriesOfLogReturnList[i].removeFirst();
						}
					}
				}
			} catch (Exception e)
			...
			for (int i = 0; i < numberOfStates; i++)
			{
				String output = percentFormatter
						.format(timeSeriesOfLogReturnList[i].getLast().getY());
				LogReturnDataLabel[i].setText(output);
			}
