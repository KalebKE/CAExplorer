...
/**
 * Finds the percentage of each state at each generation with a particular
 * neighborhood configuration with nearest neighbors.
 * 
 * @author David Bahr
 * @author Kaleb Kircher
 * @author Maria Molina
 */
public class AA1stDegreeInfluence extends Analysis implements ActionListener
{
	...
	private static final String ANALYSIS_NAME = "Alcohol 1st Degree";
	...
	private static final String INFO_MESSAGE = "Calculates the number and percentage "
			+ "of cells that have each state with a specief neighborhood configuration.";
	 ...
	 */
	public AA1stDegreeInfluence(boolean minimalOrLazyInitialization)
	{
	...
	/**
	 * Counts and displays the number of occupied cells with a particular
	...
	protected void analyze(Lattice lattice, Rule rule, int generation)
	{
				...
				&& (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			while(cellIterator.hasNext())
			{
		        ...
		        // Find the cells that are in state one with one nearest neighbor in state
		        // 1. THIS PART OF THE CODE can be changed to analyze which ever configuration
				// one wants to see.
				if(numberOfEachState[1] == 0 && cell.toInt() == 1)
		        {
