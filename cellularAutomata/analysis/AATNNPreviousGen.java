...
/**
 * Finds the percentage of each state with the a specified neighborhood configuration
 * in the previous generation. Uses nearest neighbors and next-nearest neighbors.
 * 
 * @author David Bahr
 * @author Kaleb Kircher
 * @author Maria Molina
 */
public class AATNNPreviousGen extends Analysis implements ActionListener
{
	...
	private static final String ANALYSIS_NAME = "Cell Configuration";
	 ...
	 */
	public AATNNPreviousGen(boolean minimalOrLazyInitialization)
	{
	...
	/**
	 * Counts and displays the number of cells in state 1 with a specified neighborhood
	 * configuration. The neighborhood configuration must be specified for number of 
	 * nearest neighbors and number of next nearest neighbors.
	 * 
	 * @param lattice
	 *            The CA lattice.
	 * @param rule
	...
	protected void analyze(Lattice lattice, Rule rule, int generation)
	{
				...
				&& (numberOfStates <= MAX_NUMBER_OF_STATES))
		{
			...
			while (cellIterator.hasNext())
			{
				...
				// neighbors are assigned randomly to each cell so the data structure 
				// that stores the values must be capable of dynamically resizing 
				...
				{
					nextNearest[i].add(lattice.getNeighbors(neighboringCells[i]));
				}
				
				// a LinkedList used to store the states of every next nearest neighbor
				LinkedList<Integer> templist = new LinkedList<Integer>();
				
				// counter is used to keep track of the indices
				int count = 0;

				// loop takes each neighbor's neighbor Cell[] array and transfers it to a
				// temporary array.
				for (int i = 0; i < neighboringCells.length; i++)
				{
					// temp array
					...
					for (int j = 0; j < temp[i].length; j++)
					{
						if(generation > 2)
						{
						templist.add(count, (Integer)temp[i][j].getPreviousState()
								.toInt());
						count++;
						}
					}
				...
				//stores how many cells have each state in a neighbors neighborhood
				...
				// increments the total number of drinking candidates when there is a
				// neighbor in state 1 and all next nearest neighbors in state 0.
				// THIS PART OF THE CODE can be changed to analyze which ever configuration
				// one wants to see.
				if (numberOfEachState[1] == 1 && numberOfEachStateNN[1] == 0)
				{
					totalDrinkingCandidates++;
				}

				// Looking for all of the cells that are in state 1 and have a
				// nearest neighbor that is in state 1 and next nearest neighbor in
				// state zero. THIS PART OF THE CODE can be changed to analyze which ever configuration
				// one wants to see.
				if (cell.toInt() == 1 && numberOfEachState[1] == 1
						&& numberOfEachStateNN[1] == 0)
				{
			...
			for (int i = 0; i < percentOccupiedByState.length; i++)
			{
				percentOccupiedByState[i] = (double) numberOccupiedByState[i]
						/ (double) totalDrinkingCandidates;
			}
