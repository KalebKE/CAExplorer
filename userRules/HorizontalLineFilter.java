 ...
 * @author David Bahr
 */
public class HorizontalLineFilter extends IntegerRuleTemplate
{
    /**
     ...
     * private.)
     */
    public static final String RULE_NAME = "HoriztontalLineFilter";
     ...
     */
    public HorizontalLineFilter(boolean minimalOrLazyInitialization)
    {
    ...
    protected int integerRule(int cell, int[] neighbors, int numStates,
        int generation)
    {
       int sum =  1*neighbors[0] + 1*neighbors[1]+1*neighbors[3]+
       0*neighbors[4] + -1*neighbors[5]+ -1*neighbors[6]+ -1*neighbors[7]+ 0*neighbors[0]+0*cell;

       int newSum = Math.abs(sum);
       if (newSum >1 )
       {
    	   newSum = 1;
       }
