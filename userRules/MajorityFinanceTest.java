 ...
 * @author David Bahr
 */
public class MajorityFinanceTest extends IntegerRuleTemplate
{
	...
	private static final String RULE_NAME = "Market Test";
	...
	// indicates if the number of neighbors has been initialized
	private static volatile boolean filled = false;

	private static volatile int numOpinionLeaders = 0;
	...
	// the opinion leader will change opinion with this probability (after all
	// other social network-based decisions have been made)
	private static volatile double[] probabilityOfSpontaneousOpinionChangeOL = null;
	...
	// a local copy of the probability, updated once per generation
	private static volatile double[] probabilityOfSpontaneousOpinionChangeOLLocalCopy = null;
	...
	// slider that sets the probability of opinion change
	private static JSlider[] probSliderOL = null;
	 ...
	 */
	public MajorityFinanceTest(boolean minimalOrLazyInitialization)
	{
		...
		if (!minimalOrLazyInitialization)
		{
					...
					|| forceSliderLabelOL.length != numStates)
			{
				...
				// create the opinion leader spontaneous opinion change
				// probabilities
				if (probabilitySliderLabelOL == null
						|| probabilitySliderLabelOL.length != numStates)
				{
					// force the additional properties panel to be recreated
					additionalPropertiesPanel = null;

					probabilitySliderLabelOL = new JLabel[numStates];
					probabilityOfSpontaneousOpinionChangeOL = new double[numStates];
					for (int i = 0; i < probabilityOfSpontaneousOpinionChangeOL.length; i++)
					{
						probabilityOfSpontaneousOpinionChangeOL[i] = DEFAULT_PROBABILITY;
					}

					probabilityOfSpontaneousOpinionChangeOLLocalCopy = new double[numStates];
					for (int i = 0; i < probabilityOfSpontaneousOpinionChangeOLLocalCopy.length; i++)
					{
						probabilityOfSpontaneousOpinionChangeOLLocalCopy[i] = DEFAULT_PROBABILITY;
					}
				}
	 ...
	 * biological imperatives.
	 */
	private JSlider[] createRandomOpinionSlider()
	{
		Fonts fonts = new Fonts();

		JSlider[] probSliders = new JSlider[numStates];
		...
		for (int i = 0; i < numStates; i++)
		{
			...
			String output = longDecimalFormatter
					.format(probabilityOfSpontaneousWeightGain);
			probabilitySliderLabelOL[i] = new JLabel("probability = " + output);
			probabilitySliderLabelOL[i].setFont(fonts.getBoldFont());
			...
			JSlider probSlider = new JSlider(
					0,
					MAX_SLIDER_VALUE,
					(int) (MAX_SLIDER_VALUE * (probabilityOfSpontaneousOpinionChangeOL[i])));
			probSlider.addChangeListener(new SliderListener());
			probSlider.setToolTipText(PROBABILITY_TIP);
			...
			for (int j = 0; j <= numTickMarks; j++)
			{
				double labelValue = j * (1.0 / numTickMarks);
				sliderLabelTable.put(new Integer(j * majorTickSpacing),
						new JLabel(""
								+ shortDecimalFormatter.format(labelValue)));
			}
	...
	{
		JSlider[] temperatureSliders = new JSlider[numStates];
		...
		for (int i = 0; i < numStates; i++)
		{
			...
			temperatureSlider.setLabelTable(sliderLabelTable);
			temperatureSlider.setPaintLabels(true);
			temperatureSliders[i] = temperatureSlider;
		}
	...
	private JSlider[] createUpDownNoiseSliderOL()
	{
		...
		JSlider[] upDownNoiseSliders = new JSlider[numStates];
		...
		for (int i = 0; i < numStates; i++)
		{
			...
			upDownNoiseSlider.setLabelTable(sliderLabelTable);
			upDownNoiseSlider.setPaintLabels(true);
			upDownNoiseSliders[i] = upDownNoiseSlider;
		}

		return upDownNoiseSliders;
	}
			...
			int numStates, int generation)
	{
		...
		// do it only for the first iteration
		if (generation >= 1)
		{
			filled = true;
		}
		...
		if (socialForce == null || socialForceLocalCopy == null
				|| MajorityFinanceTest.numStates != numStates)
		{
			// so we can use this elsewhere
			MajorityFinanceTest.numStates = numStates;
		...
		if (probabilityOfSpontaneousOpinionChangeOL == null
				|| probabilityOfSpontaneousOpinionChangeOLLocalCopy == null
				|| MajorityFinanceTest.numStates != numStates)
		{
			// so we can use this elsewhere
			MajorityFinanceTest.numStates = numStates;
		...
		if (currentGeneration != generation)
		{
			...
			for (int i = 0; i < socialForceLocalCopy.length; i++)
			{
				...
				probabilityOfSpontaneousOpinionChangeOLLocalCopy[i] = probabilityOfSpontaneousOpinionChangeOL[i];
		...
		System.out.println(numOpinionLeaders);
		
		if (numOpinionLeaders != 0)
		{
			if (numCellNeighbors >= numNeighbors.get(numNeighbors.size()
					- numOpinionLeaders))
			{
		...
		// if the temp is 0, then the cell's all keep their current value
		// (no
	...
	public JPanel createAdditionalPropertiesPanel()
	{
		...
		for (int state = 0; state < numStates; state++)
		{
			output = longDecimalFormatter
					.format(probabilityOfSpontaneousWeightGain);
			probabilitySliderLabelOL[state] = new JLabel("probability = "
					+ output);
			probabilitySliderLabelOL[state].setFont(fonts.getBoldFont());
			...
			for (int i = 0; i < numStates; i++)
			{
				...
				// set the color of the label to be the same as the color of the
				// state it represents
				stateColor = this.getCompatibleCellStateView().getDisplayColor(
		...
		Border titledBorder8 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Social Force", TitledBorder.LEFT,
				TitledBorder.DEFAULT_POSITION, titleFont, titleColor);
		...
		// GBC.WEST).setInsets(1, 3, 3, 3));
		// row++;
		opinionPanel.add(opinionScrollPane[1], new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(10, 1, 10, 1));
		...
		// create a scroll pane to hold all the opinion leader force sliders
		JSlider[] tempSliders = createTemperatureSliderOL();

		// create panel for the temperatureOL
		Font titleFontOL = new Fonts().getItalicSmallerFont();
		Color titleColorOL = Color.BLUE;
		Border outerEmptyBorderOL = BorderFactory.createEmptyBorder(0, 3, 2, 3);
		Border titledBorderOL = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Temperature", TitledBorder.LEFT,
				TitledBorder.DEFAULT_POSITION, titleFontOL, titleColorOL);
		JPanel temperaturePanelOL = new JPanel(new GridBagLayout());
		temperaturePanelOL.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorderOL, titledBorderOL));
		row = 0;
		// temperaturePanelOL.add(temperatureDescriptionLabel, new GBC(1, row)
		// .setSpan(5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(
		// GBC.WEST).setInsets(1, 3, 3, 3));
		// row++;
		temperaturePanelOL.add(tempSliders[0], new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1));
		row++;
		temperaturePanelOL.add(temperatureSliderLabel, new GBC(1, row).setSpan(
				5, 1).setFill(GBC.VERTICAL).setWeight(1.0, 1.0).setAnchor(
				GBC.CENTER).setInsets(1));
		// create a scroll panel for all the opinion components
		JPanel allTempPanels = new JPanel(new GridBagLayout());
		row = 0;
		allTempPanels.add(temperaturePanelOL, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));

		JScrollPane tempComponentsScroller = new JScrollPane(allTempPanels);
		int tempScrollPaneWidth = width;
		int tempScrollPaneHeight = height / 8;
		tempComponentsScroller.setPreferredSize(new Dimension(
				tempScrollPaneWidth, tempScrollPaneHeight));
		tempComponentsScroller.setMinimumSize(new Dimension(
				tempScrollPaneWidth, tempScrollPaneHeight));
		tempComponentsScroller.setMaximumSize(new Dimension(
				tempScrollPaneWidth, tempScrollPaneHeight));
		tempComponentsScroller
				.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		...
		Border titledBorder9 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Unbiased Noise", TitledBorder.LEFT,
				TitledBorder.DEFAULT_POSITION, titleFont, titleColor);
		JPanel noisePanelOL = new JPanel(new GridBagLayout());
		noisePanel.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorder9, titledBorder9));
		row = 0;
		// noisePanelOL.add(noiseDescriptionLabel, new GBC(1, row).setSpan(5, 1)
		// .setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
		// .setInsets(1, 3, 3, 3));
		// row++;
		noisePanelOL.add(noiseSliderOL[0], new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1));
		row++;
		noisePanelOL.add(noiseSliderLabelOL[0], new GBC(1, row).setSpan(5, 1)
		...
		Border titledBorder10 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Up-Down Noise", TitledBorder.LEFT,
				TitledBorder.DEFAULT_POSITION, titleFontOL, titleColorOL);
		JPanel upDownNoisePanelOL = new JPanel(new GridBagLayout());
		upDownNoisePanel.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorder10, titledBorder10));
		row = 0;
		// upDownNoisePanelOL.add(upDownNoiseDescriptionLabel, new GBC(1, row)
		// .setSpan(5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(
		// GBC.WEST).setInsets(1, 3, 3, 3));
		// row++;
		upDownNoisePanelOL.add(upDownNoiseSliderOL[0], new GBC(1, row).setSpan(
				5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1));
		row++;
		upDownNoisePanelOL.add(upDownNoiseSliderLabelOL[0], new GBC(1, row)
				.setSpan(5, 1).setFill(GBC.VERTICAL).setWeight(1.0, 1.0)
				.setAnchor(GBC.CENTER).setInsets(1));

		// create a scroll panel for all the noise components
		JPanel allNoisePanelsOL = new JPanel(new GridBagLayout());
		row = 0;
		allNoisePanelsOL.add(noisePanelOL, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));
		row++;
		allNoisePanelsOL.add(upDownNoisePanelOL, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1));
		// row++;
		// allNoisePanelsOL.add(probabilityPanel, new GBC(1, row).setSpan(5, 1)
		// .setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
		// .setInsets(1));

		JScrollPane noiseComponentsScrollerOL = new JScrollPane(
				allNoisePanelsOL);
		scrollPaneWidth = width;
		scrollPaneHeight = height / 8;
		noiseComponentsScrollerOL.setPreferredSize(new Dimension(
				scrollPaneWidth, scrollPaneHeight));
		noiseComponentsScrollerOL.setMinimumSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		noiseComponentsScrollerOL.setMaximumSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		noiseComponentsScrollerOL
				.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);

		Border outerEmptyBorder11 = BorderFactory.createEmptyBorder(0, 3, 2, 3);
		Border titledBorder11 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Noise", TitledBorder.LEFT,
				TitledBorder.DEFAULT_POSITION, titleFont, titleColor);
		JPanel allNoisePanelOL = new JPanel(new GridBagLayout());
		allNoisePanel.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorder11, titledBorder11));
		row = 0;
		allNoisePanelOL.add(noiseComponentsScrollerOL, new GBC(1, row).setSpan(
				5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));

		// ////////////////////////////////////////////////////////////////////

		Border outerEmptyBorder12 = BorderFactory.createEmptyBorder(0, 3, 2, 3);
		Border titledBorder12 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Opinion Leaders For State #",
				TitledBorder.LEFT, TitledBorder.DEFAULT_POSITION, titleFont,
				titleColor);
		JPanel allOpinionPanel = new JPanel(new GridBagLayout());
		allOpinionPanel.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorder12, titledBorder12));
		row = 0;
		allOpinionPanel.add(opinionComponentsScroller, new GBC(1, row).setSpan(
				5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));
		row++;
		allOpinionPanel.add(tempComponentsScroller, new GBC(1, row).setSpan(5,
				1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));
		row++;
		allOpinionPanel.add(noiseComponentsScrollerOL, new GBC(1, row).setSpan(
				5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));
