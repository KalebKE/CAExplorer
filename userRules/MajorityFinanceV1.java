 ...
 */
public class MajorityFinanceV1 extends IntegerRuleTemplate
{
	...
	private static final int NUM_ATTITUDES = 3;
	...
	private static final String RULE_NAME = "Market Model V1";
	...
	// a calculation of the temperature that should be added or subtracted to
	// the
	// base temperature of the Risk Adverse Traders
	private static volatile double riskAdverseSocialTemperature[];

	// a calculation of the force that should be added or subtracted to
	// the
	// base force of the Risk Adverse Traders
	private static volatile double riskAdverseSocialForce[];

	// a calculation of the temperature that should be added or subtracted to
	// the
	// base temperature of the Risk Taking Traders
	private static volatile double riskTakerSocialTemperature[];

	// a calculation of the force that should be added or subtracted to
	// the
	// base force of the Risk Taking Traders
	private static volatile double riskTakerSocialForce[];
	...
	private static String opinionLeaderAttitude[];
	 ...
	 */
	public MajorityFinanceV1(boolean minimalOrLazyInitialization)
	{
		...
		if (!minimalOrLazyInitialization)
		{
			...
			riskAdverseSocialTemperature = new double[numStates];
			riskAdverseSocialForce = new double[numStates];
			riskTakerSocialTemperature = new double[numStates];
			riskTakerSocialForce = new double[numStates];
			...
			{
				riskAdverseSocialTemperature[i] = 0;
				fundamentalValue[i] = 50;
				currentValue[i] = 50;
				transactionQuantity[i] = 0;
				previousQuantity[i] = 1000;
				previousValue[i] = 50;
			...
			int numStates, int generation)
	{
		...
		if (currentGeneration != generation)
		{
			...
			riskAdverseSocialTemperature();
			riskTakerSocialTemperature();
			riskAdverseSocialForce();
			riskTakerSocialForce();
		}
				...
				&& socialTemperatureLocalCopy != 0.0)
		{
			...
			String attitude = opinionLeaderAttitude[Arrays.binarySearch(
					opinionLeaders,
					(opinionLeaders.length - numOpinionLeaders),
					(opinionLeaders.length), neighbors.length)];

			for (int i = 0; i < numStates; i++)
			{
				if (cell == i && attitude.equals("A"))
				{
					...
					for (int j = 0; j < numStates; j++)
					{
						...
						prob[i][j] = Math
								.pow(
										Math.E,
										(socialForceOLLocalCopy[0][j] - riskAdverseSocialForce[j])
				...
				if (cell == i && attitude.equals("T"))
				{
					...
					for (int j = 0; j < numStates; j++)
					{
						...
						prob[i][j] = Math
								.pow(
										Math.E,
										(socialForceOLLocalCopy[2][j] + riskTakerSocialForce[j])
												/ (socialTemperatureOLLocalCopy[2])
												* Math
														.pow(
																((double) numberOfEachState[i])
																		/ ((double) neighbors.length + 1),
																1.0 / (socialTemperatureOLLocalCopy[2])));
					}
				...
				if (cell == i && attitude.equals("N"))
				{
					...
					for (int j = 0; j < numStates; j++)
					{
						...
						prob[i][j] = Math
								.pow(
										Math.E,
										(socialForceOLLocalCopy[1][j] + ((socialForceOLLocalCopy[0][j] + socialForceOLLocalCopy[2][j]) / 2))
												/ (socialTemperatureOLLocalCopy[1])
												* Math
														.pow(
																((double) numberOfEachState[i])
																		/ ((double) neighbors.length + 1),
																1.0 / (socialTemperatureOLLocalCopy[1])));
					}
	...
	public void calculatePricing()
	{
		...
		for (int i = 0; i < currentQuantity.length; i++)
		{
			...
			currentValue[i] = currentValue[i] + ((0.1)*r.nextDouble())
	...
	public void riskAdverseSocialTemperature()
	{
		for (int i = 0; i < riskAdverseSocialTemperature.length; i++)
		{
			riskAdverseSocialTemperature[i] = .00001 * Math.log(currentValue[i]
					/ fundamentalValue[i]);
		}
	}

	public void riskAdverseSocialForce()
	{
		for (int i = 0; i < riskAdverseSocialForce.length; i++)
		{
			riskAdverseSocialForce[i] = 0.0001 * Math.log(currentValue[i]
					/ fundamentalValue[i]);
		}
	}

	public void riskTakerSocialTemperature()
	{
		for (int i = 0; i < riskTakerSocialTemperature.length; i++)
		{
			riskTakerSocialTemperature[i] = .01 * Math.log(currentValue[i]
					/ previousValue[i]);
		}
	}

	public void riskTakerSocialForce()
	{
		for (int i = 0; i < riskTakerSocialForce.length; i++)
		{
			riskTakerSocialForce[i] = 1 * Math.log(currentValue[i]
					/ previousValue[i]);
		}
	...
	public void initializeOpinionLeaders()
	{
		...
		// attitudes for opinion leaders risk Adverse, risk Neutral,
		// risk Taker
		String[] tempString =
		{ "A", "N", "T" };
		...
		// initialize the opinion leaders attitude array
		opinionLeaderAttitude = new String[temporaryObject.length];
		...
		for (int i = 0; i < opinionLeaders.length; i++)
		{
			...
			// assign a trading attitude to each cell indexed with
			// opinionLeaders
			//opinionLeaderAttitude[i] = tempString[r.nextInt(3)];
			
			opinionLeaderAttitude[i] = "A";
		}
	}
	...
	public void initializeVariables()
	{
		if (socialForce == null || socialForceLocalCopy == null
				|| MajorityFinanceV1.numStates != numStates)
		{
