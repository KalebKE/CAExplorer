...
public class KalebRule6 extends MultiGenerationIntegerRuleTemplate
{
	/**
	 ...
	 */
	private static final String RULE_NAME = "KalebRule6";
	 ...
	 */
	public KalebRule6(boolean minimalOrLazyInitialization)
	{
		super(minimalOrLazyInitialization);
	}

	/**
     * This is how the model was constructed:
     * The state of cell represents a behavior. Most of the time, the cell wants to
     * act like its neighbors. A cell looks at its neighbors current behavior, and their previous behavior,
     * do decide what its new state will be. 
	 * The state of a cell is calculated based on two factors when using a square lattice.
	 * The first is that a cell tries to be likes its neighbors, most of the
	 * time. However, there is a chance that a cell may be defiant and will chose
	 * a completely random state. Randomness is required to drive the model, otherwise
	 * it wants to to find a state of equilibrium, which it does, and nothing happens.
	 * The second factor is a universal probability that a individual state (behavior) 
	 * will be adopted by a cell. Weight matrixes are used and randomly selected 
	 * to represent how people "see" for "feel" about a given behavior that day. The cell
	 * looks at all its neighbors and counts the number of times each behavior occurs in the
	 * neighborhood. The number of behavior occurrences is then multiplied by the weight matrix. We call
	 * this value alpha. Finally, the cell selects the state (behavior) that has the highest
	 * alpha. Unless it randomly selects another state.
	 * 
	 * @param cell
