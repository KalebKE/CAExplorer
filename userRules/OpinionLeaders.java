 ...
 * @author David Bahr
 */
public class OpinionLeaders extends IntegerRuleTemplate
{

	// the rebellion leaders will be selected from this population percentile
	// and larger.
	private static final double DEFAULT_OPINION_LEADER_PERCENTILE = 0.0;

	// the rebellion leaders will be selected with this default probability.
	private static final double DEFAULT__REBELLION_LEADER_PROBABILITY = 0.0;
	...
	// the max percentile of an opinion leader. Values can be 0 to 1.
	private static final double MAX_OPINION_LEADER_PERCENTILE = 1.0;

	// the max probability of a rebellion leader. Values can be 0 to 1.
	private static final double MAX_REBELLION_LEADER_PROBABILITY = 1.0;
	...
	// the min percentile of an opinion leader. Values can be 0 to 1.
	private static final double MIN_OPINION_LEADER_PERCENTILE = 0.0;

	// the min probability of a rebellion leader. Values can be 0 to 1.
	private static final double MIN_REBELLION_LEADER_PROBABILITY = 0.0;
	...
	// tool tip for the opinion leader percentile
	private static final String OPINION_LEADER_PERCENTILE_TIP = "The minimum population "
			+ "percentile a cell must be in to become an opinion leader.";

	// tool tip for the rebellion leader probability
	private static final String REBELLION_LEADER_PROBABILITY_TIP = "The probability that an "
			+ "opinion leader will become a rebellion leader.";
	...
	private static final String RULE_NAME = "Opinion Leaders";
	...
	private static final String BEST_RESULTS = "<html> <body><b>"
			...
			+ "<b>For Best results with opinion leaders</b>, use a small-world lattice"
			+ "with at least 5 states and a random distribution.  "
	...
	// cells with a population above this percentile will be opinion leaders
	private static volatile double[] percentileOfOpinionLeader = null;

	// a local copy of the percentile, updated once per generation
	private static volatile double[] percentileOfOpinionLeaderLocalCopy = null;

	// opinion leaders will become rebellion leaders with this probability
	private static volatile double[] probabilityOfRebellionLeader = null;

	// a local copy of the probability, updated once per generation
	private static volatile double[] probabilityOfRebellionLeaderLocalCopy = null;
	...
	// label that displays the minimum population percentile for opinion leaders
	private static JLabel[] percentileLeaderSliderLabel = null;

	// label that displays the probability of rebellion leaders
	private static JLabel[] probabilityLeaderSliderLabel = null;
	...
	private static JSlider[] rebellionProbSlider = null;

	// slider that sets the probability of weight gain
	private static JSlider[] percentileSlider = null;
	 ...
	 */
	public OpinionLeaders(boolean minimalOrLazyInitialization)
	{
		...
		if (!minimalOrLazyInitialization)
		{
			...
			// so we can use this elsewhere
			int numColumns = CurrentProperties.getInstance().getNumColumns();
			...
			// create the social forces (only if necessary)
			if (percentileLeaderSliderLabel == null
					|| percentileLeaderSliderLabel.length != numStates)
			{
				// force the additional properties panel to be recreated
				additionalPropertiesPanel = null;

				percentileLeaderSliderLabel = new JLabel[numStates];
				percentileOfOpinionLeader = new double[numStates];
				for (int i = 0; i < percentileOfOpinionLeader.length; i++)
				{
					percentileOfOpinionLeader[i] = 0.0;
				}

				percentileOfOpinionLeaderLocalCopy = new double[numStates];
				for (int i = 0; i < percentileOfOpinionLeaderLocalCopy.length; i++)
				{
					percentileOfOpinionLeaderLocalCopy[i] = 0.0;
				}
			}

			// create the social forces (only if necessary)
			if (probabilityLeaderSliderLabel == null
					|| probabilityLeaderSliderLabel.length != numStates)
			{
				// force the additional properties panel to be recreated
				additionalPropertiesPanel = null;

				probabilityLeaderSliderLabel = new JLabel[numStates];
				probabilityOfRebellionLeader = new double[numStates];
				for (int i = 0; i < probabilityOfRebellionLeader.length; i++)
				{
					probabilityOfRebellionLeader[i] = 0.0;
				}

				probabilityOfRebellionLeaderLocalCopy = new double[numStates];
				for (int i = 0; i < probabilityOfRebellionLeader.length; i++)
				{
					probabilityOfRebellionLeader[i] = 0.0;
				}
	...
	/**
	 * Create a scroll panel that holds all the percentile sliders.
	 * 
	 * @param width
	 *            The width of the additional properties panel.
	 * @param height
	 ...
	 * @return scrollPane of sliders.
	 */
	private JScrollPane createOpinionLeaderSliderPanel(int width, int height)
	{
		JPanel sliderPanel = new JPanel(new GridBagLayout());
		...
		for (int state = 0; state < numStates; state++)
		{
			...
			// percentile label
			row++;
			sliderPanel.add(percentileLeaderSliderLabel[state], new GBC(1, row)
					.setSpan(5, 1).setFill(GBC.VERTICAL).setWeight(1.0, 1.0)
					.setAnchor(GBC.CENTER).setInsets(verticalSpace, 1, 0, 1));

			// percentile slider
			row++;
			sliderPanel.add(percentileSlider[state], new GBC(1, row).setSpan(5,
		...
		JScrollPane percentileScroller = new JScrollPane(sliderPanel);
		int scrollPaneWidth = width;
		int scrollPaneHeight = (int) (height * 0.275);
		percentileScroller.setPreferredSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		percentileScroller.setMinimumSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		percentileScroller.setMaximumSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		percentileScroller
				.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);

		return percentileScroller;
	}

	/**
	 * Create the sliders for the social forces (one slider per state).
	 */
	private JSlider[] createPercentileSliders()
	{
		JSlider[] allForceSliders = new JSlider[numStates];
		...
		for (int state = 0; state < numStates; state++)
		{
			...
			JSlider slider = new JSlider(
					...
					(int) (MAX_SLIDER_VALUE
							* (percentileOfOpinionLeader[state] - MIN_OPINION_LEADER_PERCENTILE) / (MAX_OPINION_LEADER_PERCENTILE - MIN_OPINION_LEADER_PERCENTILE)));
			slider.addChangeListener(new SliderListener());
			slider.setToolTipText(OPINION_LEADER_PERCENTILE_TIP);
			...
			// the hash table of labels
			Hashtable sliderLabelTable = new Hashtable();
			double maxLabelValue = MAX_OPINION_LEADER_PERCENTILE;
			double minLabelValue = MIN_OPINION_LEADER_PERCENTILE;
			for (int i = 0; i <= numTickMarks; i++)
			{
				double labelValue = minLabelValue
						+ (i * ((maxLabelValue - minLabelValue) / numTickMarks));
	...
	/**
	 * Create a scroll panel that holds all the rebellion leader probability
	 * sliders.
	 * 
	 * @param width
	 *            The width of the additional properties panel.
	 ...
	 * @return scrollPane of sliders.
	 */
	private JScrollPane createRebellionLeaderSliderPanel(int width, int height)
	{
		JPanel sliderPanel = new JPanel(new GridBagLayout());
		...
		for (int state = 0; state < numStates; state++)
		{
			...
			// percentile label
			row++;
			sliderPanel.add(probabilityLeaderSliderLabel[state],
			...
			// percentile slider
			row++;
			sliderPanel.add(rebellionProbSlider[state], new GBC(1, row)
		...
		JScrollPane rebellionScroller = new JScrollPane(sliderPanel);
		int scrollPaneWidth = width;
		int scrollPaneHeight = (int) (height * 0.275);
		rebellionScroller.setPreferredSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		rebellionScroller.setMinimumSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		rebellionScroller.setMaximumSize(new Dimension(scrollPaneWidth,
				scrollPaneHeight));
		rebellionScroller
				.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);

		return rebellionScroller;
	}

	/**
	 * Create the sliders for the social forces (one slider per state).
	 */
	private JSlider[] createRebellionSliders()
	{
		JSlider[] allForceSliders = new JSlider[numStates];
		...
		for (int state = 0; state < numStates; state++)
		{
			...
			JSlider slider = new JSlider(
					...
					MAX_SLIDER_VALUE,
					(int) (MAX_SLIDER_VALUE
							* (probabilityOfRebellionLeader[state] - MIN_REBELLION_LEADER_PROBABILITY) / (MAX_REBELLION_LEADER_PROBABILITY - MIN_REBELLION_LEADER_PROBABILITY)));
			slider.addChangeListener(new SliderListener());
			slider.setToolTipText(REBELLION_LEADER_PROBABILITY_TIP);
			...
			// the hash table of labels
			Hashtable sliderLabelTable = new Hashtable();
			double maxLabelValue = MAX_REBELLION_LEADER_PROBABILITY;
			double minLabelValue = MIN_REBELLION_LEADER_PROBABILITY;
			for (int i = 0; i <= numTickMarks; i++)
			{
				double labelValue = minLabelValue
						+ (i * ((maxLabelValue - minLabelValue) / numTickMarks));
			...
			int numStates, int generation)
	{
		...
		if (!filled)
		{
				numNeighbors.add(neighbors.length);
		}
		...
		if (socialForce == null || socialForceLocalCopy == null
				|| OpinionLeaders.numStates != numStates)
		{
			// so we can use this elsewhere
			OpinionLeaders.numStates = numStates;
		...
		if (percentileOfOpinionLeader == null
				|| percentileOfOpinionLeaderLocalCopy == null
				|| OpinionLeaders.numStates != numStates)
		{
			// so we can use this elsewhere
			OpinionLeaders.numStates = numStates;
		...
		if (probabilityOfRebellionLeader == null
				|| probabilityOfRebellionLeader == null
				|| OpinionLeaders.numStates != numStates)
		{
			// so we can use this elsewhere
			OpinionLeaders.numStates = numStates;
			...
			probabilityOfRebellionLeaderLocalCopy = new double[numStates];
			for (int i = 0; i < probabilityOfRebellionLeaderLocalCopy.length; i++)
			{
				probabilityOfRebellionLeaderLocalCopy[i] = 0.0;
			}
		}
		...
		if (currentGeneration != generation)
		{
			...
			for (int i = 0; i < percentileOfOpinionLeaderLocalCopy.length; i++)
			{
				percentileOfOpinionLeaderLocalCopy[i] = percentileOfOpinionLeader[i];
			}

			// ditto forces
			for (int i = 0; i < probabilityOfRebellionLeaderLocalCopy.length; i++)
			{
				probabilityOfRebellionLeaderLocalCopy[i] = probabilityOfRebellionLeader[i];
			}
		...
		for (int i = 0; i < percentileOfOpinionLeader.length; i++)
		{
			if (percentileOfOpinionLeader[i] != 0.0)
			{
				// find opinion leaders
				if (cellValue == i
						&& generation > 1
						&& neighbors.length > (getValueAtPercentile(
								percentileOfOpinionLeader[i], numNeighbors)))
				{
					// Now get a random number between 0 and 1
					double randomNumber = random.nextDouble();
					
					// randomly assign a certain percentage to rebellion leaders
					if (randomNumber > (1.0 - probabilityOfRebellionLeader[i]))
					{
			...
			//if(generation > 1 && numNeighbors.get(numNeighbors.size()-1) == neighbors.length)
			//{
				//System.out.println("REBELLION!");
				//return cellValue;
			//}
	 ...
	 */
	public double getValueAtPercentile(final double p, ArrayList<Integer> values)
	{
		// find the length of the array
		double n = (double) values.size();

		// calculate the percentile position
		double pos = 100 * (p * (n + 1) / 100);

		// find the largest whole integer value not larger than the percentile
		// position
		...
		// find the difference between the position floor and the estimated
		// position
		double dif = pos - fpos;

		// if the position is less than 1, return the first index
		if (pos < 1)
		{
			return values.get(0);
		}

		// if the position is larger than the length of the array, return the
		// last value
		if (pos >= n)
		{
		...
		// find the value just before the estimated position
		double lower = values.get(intPos - 1);

		// find the value at the estimated position
		double upper = values.get(intPos);

		// take the lower plus the position error and scale it to the difference
	...
	public JPanel createAdditionalPropertiesPanel()
	{
		...
		String rebellionLeaderDescription = "Rebellion leaders are opionion leaders "
				+ "who resist change. These tend to be experts in their field who "
				+ "have long established methodolgies and are not easily willing to change "
				+ "how they do things. These cells are the very last cells to addopt a new "
				+ "methodolgy and only after everyone else has. "
				+ "This defines the percentage of opinion leaders that will be randomly assinged "
				+ "as rebllion leaders.";
		MultilineLabel rebellionLeaderDescriptionLabel = new MultilineLabel(
				rebellionLeaderDescription);
		rebellionLeaderDescriptionLabel.setFont(fonts
				.getMorePropertiesDescriptionFont());
		...
		for (int state = 0; state < numStates; state++)
		{
			output = longDecimalFormatter
					.format(percentileOfOpinionLeader[state]);
			percentileLeaderSliderLabel[state] = new JLabel("Percentile = "
					+ output);
			percentileLeaderSliderLabel[state].setFont(fonts.getBoldFont());
			...
			Color stateColor = this.getCompatibleCellStateView()
					.getDisplayColor(new IntegerCellState(state), null,
							new Coordinate(0, 0));
			percentileLeaderSliderLabel[state].setForeground(stateColor);
		}

		for (int state = 0; state < numStates; state++)
		{
			output = longDecimalFormatter
					.format(probabilityOfRebellionLeader[state]);
			probabilityLeaderSliderLabel[state] = new JLabel("Probability = "
					+ output);
			probabilityLeaderSliderLabel[state].setFont(fonts.getBoldFont());
			...
			Color stateColor = this.getCompatibleCellStateView()
					.getDisplayColor(new IntegerCellState(state), null,
							new Coordinate(0, 0));
			probabilityLeaderSliderLabel[state].setForeground(stateColor);
		}

		// create the slider for the opinion leader population percentile.
		percentileSlider = createPercentileSliders();
		...
		// create the slider for the rebellion leader probability
		rebellionProbSlider = createRebellionSliders();
		...
		// create a scroll pane to hold all the force sliders
		JScrollPane opinionScrollPane = createOpinionLeaderSliderPanel(
				width - 30, height);

		// create panel for the opinion leaders
		Border outerEmptyBorder8 = BorderFactory.createEmptyBorder(0, 3, 2, 3);
		Border titledBorder8 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Population Percentile",
				TitledBorder.LEFT, TitledBorder.DEFAULT_POSITION, titleFont,
				titleColor);
		JPanel opinionPanel = new JPanel(new GridBagLayout());
		opinionPanel.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorder8, titledBorder8));
		row = 0;
		opinionPanel.add(opinionLeaderDescriptionLabel, new GBC(1, row)
				.setSpan(5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(
						GBC.WEST).setInsets(1, 3, 3, 3));
		row++;
		opinionPanel.add(opinionScrollPane, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(10, 1, 10, 1));

		// create a scroll pane to hold all the force sliders
		JScrollPane rebellionScrollPane = createRebellionLeaderSliderPanel(
				width - 30, height);

		// create panel for the rebellion leaders
		Border outerEmptyBorder9 = BorderFactory.createEmptyBorder(0, 3, 2, 3);
		Border titledBorder9 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Rebellion Probability",
				TitledBorder.LEFT, TitledBorder.DEFAULT_POSITION, titleFont,
				titleColor);
		JPanel rebellionPanel = new JPanel(new GridBagLayout());
		rebellionPanel.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorder9, titledBorder9));
		row = 0;
		rebellionPanel.add(rebellionLeaderDescriptionLabel, new GBC(1, row)
				.setSpan(5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(
						GBC.WEST).setInsets(1, 3, 3, 3));
		row++;
		rebellionPanel.add(rebellionScrollPane, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(10, 1, 10, 1));

		// create a scroll panel for all the opinion components
		JPanel allOpinionPanels = new JPanel(new GridBagLayout());
		row = 0;
		allOpinionPanels.add(opinionPanel, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));
		row++;
		allOpinionPanels.add(rebellionPanel, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1));

		JScrollPane opinionComponentsScroller = new JScrollPane(
				allOpinionPanels);
		int opinionScrollPaneWidth = width;
		int opinionScrollPaneHeight = height / 4;
		opinionComponentsScroller.setPreferredSize(new Dimension(
				opinionScrollPaneWidth, opinionScrollPaneHeight));
		opinionComponentsScroller.setMinimumSize(new Dimension(
				opinionScrollPaneWidth, opinionScrollPaneHeight));
		opinionComponentsScroller.setMaximumSize(new Dimension(scrollPaneWidth,
				opinionScrollPaneHeight));
		opinionComponentsScroller
				.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);

		Border outerEmptyBorder10 = BorderFactory.createEmptyBorder(0, 3, 2, 3);
		Border titledBorder10 = BorderFactory.createTitledBorder(BorderFactory
				.createEtchedBorder(), "Opinion Leaders", TitledBorder.LEFT,
				TitledBorder.DEFAULT_POSITION, titleFont, titleColor);
		JPanel allOpinionPanel = new JPanel(new GridBagLayout());
		allOpinionPanel.setBorder(BorderFactory.createCompoundBorder(
				outerEmptyBorder10, titledBorder10));
		row = 0;
		allOpinionPanel.add(opinionComponentsScroller, new GBC(1, row).setSpan(
				5, 1).setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1, 3, 3, 3));
		...
		// movement restriction
		row++;
		allComponentsPanel.add(allOpinionPanel, new GBC(1, row).setSpan(5, 1)
				.setFill(GBC.BOTH).setWeight(1.0, 1.0).setAnchor(GBC.WEST)
				.setInsets(1));
		...
		public void stateChanged(ChangeEvent e)
		{
			...
			} else
			{
				for (int state = 0; state < numStates; state++)
				{
					...
					if (e.getSource().equals(rebellionProbSlider[state])
							&& rebellionProbSlider[state] != null
							&& probabilityLeaderSliderLabel[state] != null)
					{
						// get the force value in *arbitrary units
						// provided by the slider* that's why I divide by
						// MAX_SLIDER_VALUE
						double updatedSliderValue = MIN_REBELLION_LEADER_PROBABILITY
								+ ((rebellionProbSlider[state].getValue() / (double) MAX_SLIDER_VALUE) * (MAX_REBELLION_LEADER_PROBABILITY - MIN_REBELLION_LEADER_PROBABILITY));

						// make sure the value changed
						if (probabilityOfRebellionLeader[state] != updatedSliderValue)
						{
							probabilityOfRebellionLeader[state] = updatedSliderValue;

							// change the display
							String output = longDecimalFormatter
									.format(probabilityOfRebellionLeader[state]);
							probabilityLeaderSliderLabel[state]
									.setText("Probabilty = " + output);
						}
					}
					if (e.getSource().equals(percentileSlider[state])
							&& percentileSlider[state] != null
							&& percentileLeaderSliderLabel[state] != null)
					{
						// get the force value in *arbitrary units
						// provided by the slider* that's why I divide by
						// MAX_SLIDER_VALUE
						double updatedSliderValue = MIN_OPINION_LEADER_PERCENTILE
								+ ((percentileSlider[state].getValue() / (double) MAX_SLIDER_VALUE) * (MAX_OPINION_LEADER_PERCENTILE - MIN_OPINION_LEADER_PERCENTILE));

						// make sure the value changed
						if (percentileOfOpinionLeader[state] != updatedSliderValue)
						{
							percentileOfOpinionLeader[state] = updatedSliderValue;

							// change the display
							String output = longDecimalFormatter
									.format(percentileOfOpinionLeader[state]);
							percentileLeaderSliderLabel[state]
									.setText("Percentile = " + output);
						}
					}
				}
			}
