 ...
 * 
 */
public class MarketModel3 extends IntegerRuleTemplate
{
	...
	private static final String RULE_NAME = "Market Model 3";
	...
	// ArrayList used to store the values for news of each linkedState.
	// news is defined as log(numCurrentCells/numPreviousCells)
	private static volatile ArrayList<Double> news = new ArrayList<Double>();

	// ArrayList used to store the previous number of each cells in a linked
	// state.
	private static volatile ArrayList<Double> linkedStatesOld = new ArrayList<Double>();

	// ArrayList used to store the current number of each cells in a linked
	// state.
	private static volatile ArrayList<Double> linkedStatesNew = new ArrayList<Double>();
	...
	// Used to store the maxNews value from the news List for each generation.
	private static double maxNews;

	// Used to store the minNews value from the news List for each generation.
	private static double minNews;

	// finalNews is a value that is added onto social force. It is the
	// difference between maxNews and minNews.
	// finalNews = maxNews + minNews, since minNews will ALWAYS be a negative
	// value.
	 ...
	 */
	public MarketModel3(boolean minimalOrLazyInitialization)
	{
			...
			int numStates, int generation)
	{
		...
		if (socialForce == null || socialForceLocalCopy == null
				|| MarketModel3.numStates != numStates)
		{
			// so we can use this elsewhere
			MarketModel3.numStates = numStates;
		...
		if (currentGeneration != generation)
		{
			...
			for (int i = 0; i < numLinkedStates; i++)
			{
				socialForceLocalCopy[i] += (finalNews * 3);
				System.out.println(finalNews);
			}
